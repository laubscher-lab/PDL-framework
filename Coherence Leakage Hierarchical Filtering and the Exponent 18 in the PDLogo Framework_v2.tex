...
\begin{remark}[Dynamic origin of $\varepsilon$]
\label{rem:dynamic_origin}
A direct combinatorial analysis of the valence sector shows that the static signed graph alone does not enforce $\varepsilon > 0$. For any finite signed graph, Harary's theorem guarantees the existence of a balanced sign assignment with $\Gamma = 0$: it suffices to 2-colour the vertices and assign $+1$ to intra-group edges and $-1$ to inter-group edges. For the architecture $(24, 28, 930, 10087, 11017)$, the partition $A = \{u_1, u_2\}$, $B = \{d\}$ explicitly provides such an assignment over all 70\,300 triangles of the valence sector, with $\Gamma = 0$. It follows that $\varepsilon > 0$ cannot arise from the static graph alone and must originate from additional constraints absent from the static description. Three candidate sources are: (D) \textbf{Pulsation-induced transient violations}. If the update rule $F$ requires specific edges to flip sign, mixed triangles crossing the active surface may be violated during one half-period while coherent during the other. The minimal fraction of violations averaged over a complete cycle, $\Gamma_{\mathrm{cycle}}(G) = \frac{1}{2}(\Gamma(G) + \Gamma(F(G)))$, can be strictly positive even when $\Gamma(G) = 0$. (E) \textbf{Topological frustration}. Simultaneous coupling of the proton to multiple external $(4,6)$ blocks may impose mutually incompatible sign constraints on the surface, generating irreducible odd negative cycles. (F) \textbf{Charge constraint}. Encoding the net charge $+1$ may forbid all balanced 2-colourings compatible with the valence-sea architecture. The central conjecture of the PDL gravitation programme can then be stated precisely: \begin{equation} \varepsilon = \inf_{G \in \mathcal{G}_p} \Gamma_{\mathrm{cycle}}(G) = \varepsilon_G = \left(\frac{G_N m_p^2}{\hbar c}\right)^{1/18} = 0.0075194. \label{eq:central_conjecture} \end{equation} Establishing this equality requires: (i) an explicit specification of the proton pulsation rule $F$; (ii) a computation of $\Gamma_{\mathrm{cycle}}$ for the canonical architecture; (iii) a verification that this minimum coincides with $\varepsilon_G$. These three steps constitute the priority research programme for grounding the structural bridge between combinatorial coherence leakage and the gravitational constant.
\end{remark}

\begin{remark}[Computational determination of $\varepsilon$]
\label{rem:computational_eps}
A direct numerical analysis of the valence sector confirms that the minimal closure defect of the static signed graph is zero: under the balanced Harary assignment with $A = \{u_1, u_2\}$ and $B = \{d\}$, all 70\,300 triangles of the valence sector $(n_u=24, n_d=28)$ are coherent, so $\Gamma_{\min} = 0$. This implies that $\varepsilon > 0$ cannot arise from the static graph alone but requires a dynamical constraint, as discussed in Remark~\ref{rem:dynamic_origin}.

Independently, the bridge formula established in Ref.~\cite{Universal_Coherence_Leakage_2026} provides a structurally derived expression for $\varepsilon_G$:
\begin{equation}
    \varepsilon_G 
    \;=\; \frac{\Delta r_{\mathrm{val}} \cdot r_{\mathrm{val}}^2 \cdot n_u^2}
               {R_{\mathrm{tot}} \cdot R_{\mathrm{surf}} \cdot (n_u^2-1)},
    \label{eq:eps_G_bridge}
\end{equation}
numerically verified to $10^{-5}$ relative precision. This expression shows that $\varepsilon_G$ is not a purely combinatorial invariant of the proton architecture: it contains $\Delta r_{\mathrm{val}}$, which depends on $\alpha_{\exp}$ and $\mu$. The leakage parameter $\varepsilon_G$ is therefore a mixed quantity — partly combinatorial, partly electromagnetic — that encodes precisely the structural link between the gravitational and electromagnetic sectors established by the bridge formula.
\end{remark}
