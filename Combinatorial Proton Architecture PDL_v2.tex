...
In this version, the structural representation of the fine-structure constant associated with the PDL proton has been aligned with the updated convention adopted in the dedicated derivation of $\alpha_{\mathrm{PDL}}$ and in the Schrödinger–PDL compatibility analysis.
Rather than taking the closed-form expression
\[
\alpha_{\mathrm{PDL}}^{-1} = \mu\,\varphi^{2}\,\frac{r_{\mathrm{val}}^{2}}{9}
\]
as the primary relation, we now regard
\[
\alpha_{\mathrm{PDL}} = \frac{\mu}{R_{\mathrm{surf}}^{2}}
\]
as the fundamental active-surface expression, where $\mu = m_p/m_e$ denotes the proton–electron mass ratio and $R_{\mathrm{surf}}$ is the active surface of the PDL proton.
The golden-ratio hypothesis $R_{\mathrm{surf}} = \varphi\,r_{\mathrm{val}}/3$ (with $r_{\mathrm{val}} = 930$ and $\varphi = (1+\sqrt{5})/2$) is then treated as a secondary geometric specialisation, which recasts the above into
\[
\alpha_{\mathrm{PDL}} = \mu\,\frac{9}{\varphi^{2} r_{\mathrm{val}}^{2}},
\]
and yields $\alpha_{\mathrm{PDL}}^{-1} \simeq 137.02$ in agreement with experiment.
All references to the fine-structure constant in the present article should therefore be read through this two-step interpretation: a primary active-surface relation, followed by a specific golden-ratio parametrisation of $R_{\mathrm{surf}}$ for the PDL proton.
...
\end{equation}
which gives $R_{\mathrm{surf}}\approx 501.6$ for $r_{\mathrm{val}}=930$ and $\varphi=(1+\sqrt{5})/2$.\cite{LaubscherDerivationAlpha2026,LaubscherGoldenRatioPDL2026} Within this framework, the fine-structure constant is reinterpreted as a surface-to-volume coherence fraction associated with the protonic active surface.
The primary structural ansatz is taken to be
\begin{equation}
\alpha_{\mathrm{PDL}} = \frac{\mu}{R_{\mathrm{surf}}^{2}},
\label{eq:alphaPrimaryComb}
\end{equation}
where $\mu = m_p/m_e$ denotes the proton–electron mass ratio and $R_{\mathrm{surf}}$ is the cardinality of the active surface $S_{\mathrm{act}}$.
Under the golden-ratio optimisation hypothesis, the active surface is expressed in terms of the valence content as
\begin{equation}
R_{\mathrm{surf}} = \varphi\,\frac{r_{\mathrm{val}}}{3},
\label{eq:RsurfGoldenComb}
\end{equation}
with $r_{\mathrm{val}} = 930$ and $\varphi = (1+\sqrt{5})/2$.\cite{LaubscherDerivationAlpha2026}
Combining Eqs.~\eqref{eq:alphaPrimaryComb} and~\eqref{eq:RsurfGoldenComb} yields
\begin{equation}
\alpha_{\mathrm{PDL}} = \mu\,\frac{9}{\varphi^{2} r_{\mathrm{val}}^{2}},
\label{eq:alphaValenceComb}
\end{equation}
which numerically evaluates to
$\alpha_{\mathrm{PDL}} \simeq 7.299\times 10^{-3}$,
equivalently
$\alpha_{\mathrm{PDL}}^{-1} \simeq 137.02$,
in very good agreement with the empirical value $\alpha \simeq 7.297\times 10^{-3}$ (or $\alpha^{-1} \simeq 137.036$).\cite{LaubscherDerivationAlpha2026}
...
\begin{equation}
\alpha_{\mathrm{PDL}}(\mu,r_{\mathrm{val}},R_{\mathrm{surf}}) = \frac{\mu}{R_{\mathrm{surf}}^{2}}.
\end{equation}
...
\begin{equation}
\alpha_{\mathrm{PDL}}^{-1} = \frac{R_{\mathrm{surf}}^{2}}{\mu},
\end{equation}
...
\item It supports an active surface that is consistent with the core-to-surface relation and yields a structural prediction for $\alpha$ in excellent agreement with experiments.
