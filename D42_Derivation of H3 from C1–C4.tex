...
\title{%
  ...
  \large\normalfont D42 --- Projective Dynamic Logo Framework%
}
\author{%
  C.~Laubscher%
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{D42_fig1_proof_chain_EN.png}
...
We recall the central result of~D39 (Lemmas~C3 and~C4 therein), recast here in the language needed for the main theorem.

\begin{definition}[Cross-triangle]
A \emph{cross-triangle} is a triangle formed by two vertices $e_i, e_j$ of an external $\Kfour$ block and one relation $p_k \in \Rtot$ of the proton closure, via the three edges $(e_i, e_j)$ (internal to~$\Kfour$), $(e_i, p_k)$, and $(e_j, p_k)$ (cross-edges).
\end{definition}

\begin{lemma}[Cross-triangle blindness --- D39, Lemma~C3]
\label{lem:blind}
Under axiom~C3, the $(A)\wedge(B)$ criterion evaluates cross-triangles via the cross-edge signs between $\Kfour$ and the proton. This evaluation is independent of whether the proton relation $p_k$ belongs to $\mathcal{G}_{\mathrm{val}}$ or $\mathcal{G}_{\mathrm{sea}}$: the criterion is blind to the valence/sea partition.
\end{lemma}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{D42_fig2_cross_triangles_EN.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{D42_fig3_multiscale_EN.png}
...
Cross-triangle blindness (D42-L2a) & Theorem (D39) & C3, (A)$\wedge$(B) criterion \\
...
$H_3$ (Indifference Lemma) & \textbf{Theorem} & C1--C4, D16a, D29, D39 \\
