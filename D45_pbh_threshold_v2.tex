...
% ── Abstract ──────────────────────────────────────────────
\begin{abstract}
The Projective Dynamic Logo (\PDL) framework derives all fundamental physical constants from four axioms on finite signed graphs, without presupposing spacetime, particles, or fields~\cite{D01,D21,D43v3}. A central result of the framework is that the effective gravitational coupling is density-dependent: $\Geff(N) = \sigN \cdot \GPDL$, where $\sigN = 1-(1-\kappaPDL)^{N}$ and $\kappaPDL = 310\phigold/11017 \in \mathbb{Q}(\sqrt{5})$ is an unconditional theorem of the axioms~\cite{D36,D42}. At the epoch of primordial black hole (\textsc{pbh}) formation ($N = \NCMB = 40$, derived from the neutron quintuplet without cosmological input~\cite{D27}), this coupling suppression extends \textsc{pbh} lifetimes by a factor $\sigma(40)^{-2} = 1.4007$. The resulting shift in the \textsc{pbh} survival mass threshold is $\MstarPDL = \sigma(40)^{-2/3}\,\MstarGR = 1.1189\,\MstarGR$, corresponding to an upward shift of $+11.89\%$ relative to the standard general-relativistic value $\MstarGR \approx 5.1 \times 10^{14}$\,g. This prediction implies an excess of Hawking radiation flux in the isotropic gamma-ray background (\textsc{igrb}) from \textsc{pbh}s in the mass window $[5.10,\,5.71]\times10^{14}$\,g that are still active according to \PDL\ but are predicted to have already evaporated by standard general relativity. A complete calculation of the multi-component Hawking spectrum (direct emission, secondary radiation, final-state radiation, and in-flight positron annihilation), performed with the public pipeline \textsc{GammaPBHPlotter}~\cite{Carlini2025} built on \textsc{BlackHawk}~\cite{BlackHawk} spectral tables, locates the spectral peak at $E_{\mathrm{peak}}(\MstarGR) \approx 130.1$\,MeV under standard \textsc{gr} and at $E_{\mathrm{peak}}(\MstarPDL) \approx 117.5$\,MeV under \PDL, a fractional displacement of $-9.65\%$, superseding the cruder primary-emission-only estimate ($\approx 90$--$105$\,MeV) used in earlier drafts of this prediction. The prediction is fully determined by a single unconditional theorem ($\kappaPDL$) and contains no free parameter beyond the standard-model degrees of freedom entering the Hawking spectrum. We identify the precise observational test using the public codes \textsc{BlackHawk}~\cite{BlackHawk}, \textsc{GammaPBHPlotter}~\cite{Carlini2025} and \textsc{Isatis}~\cite{Isatis} together with the Fermi-LAT \textsc{igrb} measurements of Ackermann et al.~\cite{FermiIGRB2015} and the constraint compilation of Carr et al.~\cite{Carr2021}, and invite the community to carry it out. We note that a very recent independent analysis of the same \textsc{igrb} data~\cite{Cholis2026} constrains the \textsc{pbh} abundance near this mass to $f_{\mathrm{PBH}}\lesssim10^{-10}$, which may render the present signal too faint to discriminate regardless of which cutoff is correct; this is discussed in Section~\ref{sec:discussion}.
\end{abstract}
...
Excess flux, peak at $130.1$ (\textsc{gr}) vs $117.5$\,MeV (\PDL) & Computed (full \textsc{BlackHawk}/\textsc{GammaPBHPlotter} pipeline, this document, verified) \\
Test via \textsc{BlackHawk}/\textsc{GammaPBHPlotter}/\textsc{Isatis} & Spectral computation performed (this document); comparison to Fermi-LAT \textsc{igrb} data to be completed by the community \\
\bottomrule
\end{tabular}
...
\end{align}
The peak photon energy in the Hawking spectrum scales approximately as $E_{\mathrm{peak}} \sim 5\,T_H$~\cite{MacGibbon1991} for the primary emission channel alone, giving a first-order estimate of $E_{\mathrm{peak}} \approx 93$--$104\,\mathrm{MeV}$, already within the Fermi-LAT energy range of 20\,MeV to 300\,GeV~\cite{Atwood2009}.

This estimate considers only the primary/direct emission channel and omits the secondary radiation from hadronisation and decay of unstable particles, the final-state radiation from relativistic electrons and positrons, and the in-flight annihilation of those positrons with the interstellar medium --- all of which contribute non-negligibly near the spectral peak~\cite{Carlini2025}. A complete calculation was therefore performed using the public pipeline \textsc{GammaPBHPlotter}~\cite{Carlini2025}, which combines all four components and interpolates the primary and secondary spectra from the pre-computed \textsc{BlackHawk}~\cite{BlackHawk} tables via a bivariate spline (linear in $\log M$, cubic in $\log E$, fitted to the full grid of 56 pre-rendered masses spanning $5\times10^{13}$ to $10^{19}$\,g). Evaluating the resulting total spectrum at $M=\MstarGR=5.099\times10^{14}$\,g and at $M=\MstarPDL=5.705\times10^{14}$\,g gives:
\begin{align}
  E_{\mathrm{peak}}(\MstarGR)  &\approx 130.1\,\mathrm{MeV}, \\
  E_{\mathrm{peak}}(\MstarPDL) &\approx 117.5\,\mathrm{MeV},
\end{align}
a fractional shift of $\Delta E/E = -9.65\%$ in the position of $E^2\,dN_\gamma/dE_\gamma$, both values lying comfortably within the Fermi-LAT range. The shift is close to, but more accurate than, the crude estimate above; the residual difference between $9.65\%$ and the naive expectation $1-\sigma(40)^{2/3}\approx10.6\%$ (from $T_H\propto M^{-1}$ alone) reflects the contribution of the secondary and annihilation channels to the position of the total spectral peak. This calculation supersedes the primary-only estimate used in earlier drafts of this prediction and should be taken as the reference values for any observational test.

\subsection{Nature of the signal}

The signal is not a shift in the \emph{abundance} of \textsc{pbh}s, which \PDL\ does not predict. It is a shift in the \emph{survival threshold}: the spectral peak of the Hawking contribution to the \textsc{igrb} is displaced from $E_{\mathrm{peak}}(\MstarGR) \approx 130.1$\,MeV to $E_{\mathrm{peak}}(\MstarPDL) \approx 117.5$\,MeV. In practice, if the initial \textsc{pbh} abundance $f_{\mathrm{PBH}}$ is non-zero in the window $[\MstarGR, \MstarPDL]$, one expects:
\begin{itemize}
  \item An excess of \textsc{igrb} flux at $E \lesssim 130$\,MeV relative to the \textsc{gr} prediction.
  \item The spectral shape of the excess follows the full multi-component Hawking spectrum~\cite{Carlini2025,BlackHawk} evaluated at $T_H \approx 19$--$21$\,MeV.
  \item The fractional shift in the spectral peak is $\Delta E/E \approx 9.65\%$, comparable to the Fermi-LAT spectral resolution at 100--130\,MeV ($\sim 10$--$20\%$).
\end{itemize}
...
\paragraph{Step 1 — Compute the Hawking spectra (performed in this revision).} The full multi-component photon spectrum $d^2N_\gamma/(dE\,dt)$ was computed for monochromatic \textsc{pbh} masses $M=\MstarGR=5.099\times10^{14}$\,g and $M=\MstarPDL=5.706\times10^{14}$\,g using \textsc{GammaPBHPlotter}~\cite{Carlini2025}, which combines \textsc{BlackHawk}~\cite{BlackHawk} primary and secondary spectral tables with final-state radiation and in-flight annihilation components. The resulting spectral peaks are reported in Section~\ref{sec:peak} above; the full spectra are available from the authors on request.

\paragraph{Step 2 — Normalise to the IGRB constraints (remaining).} Use \textsc{Isatis}~\cite{Isatis}, or the constraint pipeline of~\cite{Cholis2026}, to compare the computed spectra against the Fermi-LAT \textsc{igrb} measurement of Ackermann et al.~\cite{FermiIGRB2015} and the constraint compilation of Carr et al.~\cite{Carr2021}. The relevant quantity is the upper limit on $f_{\mathrm{PBH}}(M)$ in the window $[5.10, 5.71]\times10^{14}$\,g; note that~\cite{Cholis2026} already finds $f_{\mathrm{PBH}}\lesssim10^{-10}$ near this mass (Section~\ref{sec:discussion}).

\paragraph{Step 3 — Identify the discriminating observable.} The discriminating quantity is the position of the spectral peak in the 100--150\,MeV band of the \textsc{igrb}. Under \textsc{gr}, this peak occurs at $E_{\mathrm{peak}} \approx 130.1$\,MeV; under \PDL, it is displaced to $E_{\mathrm{peak}} \approx 117.5$\,MeV. A fit to the Fermi-LAT \textsc{igrb} spectrum in this band with $E_{\mathrm{peak}}$ as a free parameter constitutes a direct test of the \PDL\ prediction.
...
The \PDL\ framework predicts the \emph{position} of the \textsc{pbh} survival threshold, not the \emph{abundance} of \textsc{pbh}s in any mass window. The observational signal therefore depends on the initial \textsc{pbh} mass function, which is cosmological model-dependent and not determined by \PDL. If $f_{\mathrm{PBH}}$ is negligibly small in $[\MstarGR, \MstarPDL]$, the signal will be below any present detection threshold. In that case the prediction cannot be confirmed, but it is also not falsified. Falsification would require demonstrating that the \textsc{igrb} spectral peak is definitively located at $E_{\mathrm{peak}}^{\mathrm{GR}} \approx 130.1$\,MeV rather than at $E_{\mathrm{peak}}^{\mathrm{PDL}} \approx 117.5$\,MeV, together with a non-negligible $f_{\mathrm{PBH}}$ in the relevant window. We note that this latter condition is in tension with a very recent, independent re-analysis of the same \textsc{igrb} dataset~\cite{Cholis2026}, combined with \textsc{EGRET} and \textsc{COMPTEL} data, which constrains $f_{\mathrm{PBH}} \lesssim 10^{-10}$ for \textsc{pbh}s of mass $\sim10^{14}$\,g under standard \textsc{gr} evaporation. If this constraint also applies, essentially unchanged, to the \PDL-shifted window, the present signal may be too faint to discriminate between the two frameworks with current data, irrespective of which cutoff position is correct; this would not invalidate the prediction, only its near-term testability. We regard this as an open, and honestly reported, limitation of the present proposal rather than a difficulty to be minimised.

\paragraph{Comparison with other modified-gravity scenarios.}
Several alternative frameworks also predict modifications to the Hawking evaporation rate, including models with extra dimensions~\cite{Anchordoqui2022}, memory-burden effects~\cite{Thoss2024}, and dark charges~\cite{Korwar2023}. The \PDL\ prediction is distinguished by three features: (i) the shift is derived from a single unconditional combinatorial theorem ($\kappaPDL \in \mathbb{Q}(\sqrt{5})$), not from a new adjustable parameter; (ii) the same framework simultaneously derives $G$, $\alpha$, $\mu^*$, and the Hubble tension resolution; (iii) the shift is positive (heavier threshold) and of a specific magnitude ($+11.89\%$ in mass, $-9.65\%$ in spectral peak energy) that does not depend on any model tuning.

\paragraph{Sensitivity requirements.}
The spectral displacement $\Delta E/E \approx 9.65\%$ is at the boundary of current Fermi-LAT sensitivity at $\sim100$--$130$\,MeV. A dedicated analysis using the full 15+ years of Fermi-LAT data, focusing on the \textsc{igrb} spectrum in the 80--200\,MeV window, would provide the most constraining test, subject to the abundance caveat above. Next-generation MeV telescopes such as AMEGO-X~\cite{AMEGO} and e-ASTROGAM~\cite{eASTROGAM} would improve the sensitivity substantially, potentially resolving the spectral peak position to better than 5\%; \cite{Cholis2026} reaches the same conclusion regarding the need for next-generation instruments in this band.
...
\end{equation}
This prediction is an unconditional corollary of the density-dependent effective gravitational coupling $\Geff(N) = \sigN\cdot\GPDL$, itself proved from the \PDL\ axioms C1--C4 without any free parameter. The resulting observable, computed with the full multi-component Hawking spectrum, is a shift of the spectral peak from $\approx130.1$\,MeV (\textsc{gr}) to $\approx117.5$\,MeV (\PDL), a $-9.65\%$ displacement in the Hawking contribution to the isotropic gamma-ray background, well within the energy range accessible to Fermi-LAT.

The spectral computation has been performed in this revision using \textsc{GammaPBHPlotter}~\cite{Carlini2025} and \textsc{BlackHawk}~\cite{BlackHawk}. What remains is a direct comparison with the Fermi-LAT \textsc{igrb} data using \textsc{Isatis} or an equivalent pipeline, subject to the abundance caveat raised in Section~\ref{sec:discussion}. This is a standard numerical procedure well within the capacity of existing codes and data. We invite the community to perform this comparison and report the result, whether confirmatory or falsifying.

\begin{prediction}[Summary]
\textbf{PDL prediction D45:} The \textsc{pbh} survival mass threshold satisfies $\MstarPDL = \sigma(40)^{-2/3}\,\MstarGR = 5.706\times10^{14}$\,g. The full Hawking flux from \textsc{pbh}s in the window $[5.10,\,5.71]\times10^{14}$\,g contributes a non-zero signal to the present-day \textsc{igrb} with spectral peak at $E_{\mathrm{peak}} \approx 117.5$--$130.1$\,MeV (\PDL\ vs \textsc{gr}, computed with \textsc{GammaPBHPlotter}/\textsc{BlackHawk}). This signal is absent in standard \textsc{gr}. It can be tested with \textsc{BlackHawk} + \textsc{GammaPBHPlotter} + \textsc{Isatis} + Fermi-LAT \textsc{igrb} data, though a recent independent constraint~\cite{Cholis2026} suggests the relevant \textsc{pbh} abundance may be too small for near-term detection.
\end{prediction}
