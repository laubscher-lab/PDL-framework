...
\begin{proposition}[Mass-(in)variance of Propositions~\ref{prop:negative1} and~\ref{prop:negative2}]
\label{prop:massinvariance}
The discrepancy of Proposition~\ref{prop:negative1} is independent of $M$ by algebraic construction: writing $N=M/m_p$ and substituting into both the naive count and the Bekenstein--Hawking target,
\begin{equation}
\frac{\ln\Omegasurf^{\mathrm{naive}}}{\ln\Omegasurf^{\mathrm{target}}} = \frac{\binom{N}{2}\ln 2}{4\pi(N m_p/\MPl)^2} = \frac{\ln 2}{8\pi}\left(\frac{\MPl}{m_p}\right)^2 \approx 4.67\times10^{36},
\end{equation}
independently of $M$. This is confirmed numerically to three significant figures for $M=1\,M_\odot$, $M=6.5\times10^{9}\,M_\odot$ (M87*), $M=4.297\times10^6\,M_\odot$ (Sagittarius~A*), and $M=62\,M_\odot$ (the GW150914 remnant): the ratio is $4.68$, $4.67$, $4.68$, and $4.67\times10^{36}$ respectively. The invariance of Proposition~\ref{prop:negative1}'s discrepancy is therefore a tautological consequence of both quantities scaling as $N^2$ with fixed prefactors, not an empirical regularity requiring explanation.

By contrast, the surface-restricted count of Proposition~\ref{prop:negative2} does \emph{not} share this invariance, because $N_{\mathrm{surf}}\sim N^{2/3}$ introduces a different, $M$-dependent scaling relative to the target. Repeating the calculation of Proposition~\ref{prop:negative2} for M87* ($N\approx7.73\times10^{66}$, $N_{\mathrm{surf}}\approx3.91\times10^{44}$) gives
\begin{equation}
\ln\Omegasurf^{\mathrm{surf}}(\mathrm{M87}^*) \approx 5.30\times10^{88}~\mathrm{nats}, \qquad \ln\Omegasurf^{\mathrm{target}}(\mathrm{M87}^*) \approx 4.43\times10^{96}~\mathrm{nats},
\end{equation}
a discrepancy of $8.4\times10^{7}$, i.e.\ \textbf{7.9 orders of magnitude} --- not the 1.4 orders found for one solar mass. The apparent improvement reported in Proposition~\ref{prop:negative2} is therefore specific to the mass at which it was originally evaluated and does not constitute a generic property of the surface-restricted mechanism.
\end{proposition}

\begin{remark}
Proposition~\ref{prop:massinvariance} strengthens rather than weakens the conclusion of Proposition~\ref{prop:negative2}: not only is ``nucleon'' the wrong counting unit, but the specific areal restriction $N_{\mathrm{surf}}\sim N^{2/3}$ tested there is itself not the correct geometric prescription, since a unit that captured the right physics would be expected to leave a discrepancy of comparable order across very different black-hole masses, as Proposition~\ref{prop:negative1}'s tautological invariance illustrates for the (wrong) mechanism it rules out. Any future candidate solution to OP-D64-1 (Section~\ref{sec:open}) should be checked against at least two black-hole masses differing by several orders of magnitude before being reported as a partial success; this is now standard practice in the present author's working protocol following the cross-check performed for this revision.
\end{remark}

\begin{remark}
All three propositions above are reported as negative or partial results, in keeping with the \PDL\ programme's convention of documenting failed approaches with the same rigour as positive ones~\cite{PDL_DM_v29}. Proposition~\ref{prop:negative1} rules out one natural hypothesis, with a discrepancy now shown to be a structural tautology rather than a meaningful measurement; Proposition~\ref{prop:negative2} narrows the search at one mass but does not resolve it; Proposition~\ref{prop:massinvariance} shows that this narrowing does not survive a change of mass, and records this as a methodological requirement for future attempts.
\end{remark}
...
Mass-(in)variance of both extrapolations (Prop.~\ref{prop:massinvariance}) & Theorem (Prop.~\ref{prop:negative1} invariance, algebraic) and negative result (Prop.~\ref{prop:negative2} non-generalisation), verified on four black-hole masses & this document \\[4pt]
