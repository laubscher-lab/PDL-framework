...
\]
where \(\mu = m_p/m_e\) denotes the proton–electron mass ratio. Numerical evaluation of this expression yields \(\alpha_{\mathrm{PDL}} \simeq 7.299 \times 10^{-3}\), in agreement with the experimental value \(\alpha \simeq 7.297 \times 10^{-3}\) (equivalently \(\alpha_{\mathrm{PDL}}^{-1} \simeq 137.02\) versus \(\alpha^{-1} \simeq 137.036\)) at the level of a few \(10^{-4}\) in relative terms, and obtained without the introduction of any additional continuous fitting parameter. We investigate the domain of validity, local robustness, and intrinsic limitations of this derivation, and analyse how it can be incorporated into the conventional Schrödinger description of the hydrogen atom.
\end{abstract}
...
The Projective Dynamic Logo (PDL) framework introduces a fundamentally different conceptual basis for theoretical physics. In contrast to conventional approaches that posit, as primary structures, a spacetime manifold, classical fields, or a continuum of degrees of freedom, PDL represents physical reality as a network of minimal logical closures formalised as finite signed graphs. Informational entities are associated with the vertices of these graphs, while binary relations of concordance or discordance are encoded as signed edges. Dynamical stability is then characterised via a coherence-oriented optimisation functional defined on this graph-theoretic structure.
...
\begin{itemize}
  ...
  \item[(ii)] To obtain a closed-form expression for the fine-structure constant \(\alpha\) in terms of the proton–electron mass ratio \(\mu\), the valence content \(r_{\mathrm{val}}\), and a single geometric factor \(\varphi\), and to perform a numerical evaluation of this expression. Under the stated hypotheses, the main result is that 
  \[
  \alpha_{\mathrm{PDL}} = \mu\,\frac{9}{\varphi^2 r_{\mathrm{val}}^2}
  \]
  which yields \(\alpha_{\mathrm{PDL}} \simeq 7.299\times 10^{-3}\), in close quantitative agreement with the experimentally determined value \(\alpha \simeq 7.297\times 10^{-3}\) (equivalently \(\alpha_{\mathrm{PDL}}^{-1} \simeq 137.02\) versus \(\alpha^{-1} \simeq 137.036\)).
...
The fine-structure constant is identified with a surface coherence ratio of the proton–electron system, given by
\[
\alpha = \frac{\mu}{R_{\mathrm{surf}}^2},
\]
...
\label{thm:alpha_LDP}
Let \(r_{\mathrm{val}} = 930\) denote the valence content of the proton, and
\(\mu = m_p/m_e\) the proton--electron mass ratio.
Assume Hypotheses~\ref{hyp:alpha_surface_ratio} and \ref{hyp:phi}. Then the
fine-structure constant predicted by the LDP framework is
\[
\alpha_{\mathrm{LDP}}
...
\[
\alpha_{\mathrm{LDP}} \simeq 7.299\times 10^{-3}
\quad\text{(equivalently } \alpha_{\mathrm{LDP}}^{-1} \simeq 137.02\text{)},
\]
which agrees with the experimental value \(\alpha \simeq 7.297\times 10^{-3}\)
(\(\alpha^{-1} \simeq 137.036\)) to within a relative deviation of a few
\(10^{-4}\).
\end{theorem}
...
R_{\mathrm{surf}} = \varphi\, \frac{r_{\mathrm{val}}}{3}.
\]
Substituting this expression into the relation for \(\alpha\) gives
\[
\alpha
...
= \mu\, \frac{9}{\varphi^2 r_{\mathrm{val}}^2}.
\]
We denote this structurally defined quantity by \(\alpha_{\mathrm{LDP}}\),
thereby obtaining the claimed closed-form expression.
  ...
  and consequently
  \[
    \alpha_{\mathrm{LDP}} = \frac{\mu}{R_{\mathrm{surf}}^2}
    \simeq \frac{1836.15}{(501.59)^2} 
    \simeq 7.299\times 10^{-3} 
    \quad\text{(so that } \alpha_{\mathrm{LDP}}^{-1} \simeq 137.02\text{)}.
  \]
...
= \mu\, \frac{9}{\varphi^2 r_{\mathrm{val}}^2}.
\]
The only continuous input parameter in the construction is the empirically determined mass ratio \(\mu\); the quantities \(r_{\mathrm{val}}\) and \(\varphi\) appear, respectively, as a discrete valence parameter fixed by the assumed proton architecture and as a purely geometric optimisation factor. No further continuous fitting parameter is introduced.

From a numerical standpoint, the agreement with the experimentally measured value of \(\alpha\) (equivalently \(\alpha^{-1}\)) at the level of \(10^{-4}\) indicates that the LDP proton architecture, together with the surface-coherence hypothesis, encodes a non-trivial subset of the combinatorial constraints that underlie the electromagnetic coupling. In the remainder of the paper, we analyse the local robustness of this expression and its integration into the standard Schrödinger framework for the hydrogen atom.

\section{Local robustness of the derivation}

The structural formula obtained in Theorem~\ref{thm:alpha_LDP} shows that, once the proton architecture and the optimisation factor \(\varphi\) are specified, the fine-structure constant is given by a simple algebraic function of \(\mu\), \(r_{\mathrm{val}}\), and \(\varphi\). In this section, we examine the sensitivity of \(\alpha_{\mathrm{LDP}}^{-1}\) to small perturbations of the effective surface \(R_{\mathrm{surf}}\) and of the parameters entering its definition. Our goal is not to carry out an exhaustive statistical analysis, but rather to demonstrate that the observed numerical agreement at the \(10^{-4}\) level is consistent with physically plausible combinatorial uncertainties in the surface content.

\subsection{Sensitivity analysis}

For the purposes of this subsection, we start from the structural relation
\(\alpha = \mu / R_{\mathrm{surf}}^2\) of Hypothesis~\ref{hyp:alpha_surface_ratio} and consider its inverse,
\[
\alpha^{-1}(R_{\mathrm{surf}}) = \frac{R_{\mathrm{surf}}^2}{\mu},
\]
...
\begin{proposition}[Dependence of \texorpdfstring{\(\alpha^{-1}\)}{alpha^{-1}} on \texorpdfstring{\(R_{\mathrm{surf}}\)}{R\_surf}]
  ...
  Let \(\alpha^{-1}(R_{\mathrm{surf}}) = R_{\mathrm{surf}}^2 / \mu\) with fixed \(\mu > 0\). Then 
  \[ 
  \frac{d\,\alpha^{-1}}{d R_{\mathrm{surf}}} = \frac{2 R_{\mathrm{surf}}}{\mu} = 2\,\frac{\alpha^{-1}}{R_{\mathrm{surf}}}. 
  \]
In particular, a small relative variation \(\delta R_{\mathrm{surf}} / R_{\mathrm{surf}}\) induces, to first order, a relative variation of \(\alpha^{-1}\) given by 
  \[ 
...
\begin{proof}
Differentiating \(\alpha^{-1}(R_{\mathrm{surf}}) = R_{\mathrm{surf}}^{2}/\mu\) with respect
to \(R_{\mathrm{surf}}\) yields
\[
\frac{d\,\alpha^{-1}}{d R_{\mathrm{surf}}}
= \frac{2 R_{\mathrm{surf}}}{\mu}
...
\(R_{\mathrm{surf}}^{(0)} \approx 501.59\) obtained in Proposition~\ref{prop:Rsurf_refined}. In this subsection we keep the auxiliary parametrisation \(\alpha^{-1} = \mu / R_{\mathrm{surf}}^2\) used for sensitivity estimates, even though the primary structural relation is written in terms of \(\alpha\).
...
\[
  V_{\mathrm{eff}}(r)= -\,\alpha_{\mathrm{PDL}}\,\hbar c\,\frac{1}{r},\qquad\alpha_{\mathrm{PDL}} = \mu\,\frac{9}{\varphi^2 r_{\mathrm{val}}^2},
\]
...
Numerically, Theorem~\ref{thm:alpha_LDP} yields \(\alpha_{\mathrm{PDL}} \simeq 7.299\times 10^{-3}\) (equivalently \(\alpha_{\mathrm{PDL}}^{-1} \simeq 137.02\)), while the empirical value satisfies \(\alpha \simeq 7.297\times 10^{-3}\) (\(\alpha^{-1} \simeq 137.036\)). The resulting relative discrepancy in \(\alpha\) is of order a few \(10^{-4}\), which implies
\[
