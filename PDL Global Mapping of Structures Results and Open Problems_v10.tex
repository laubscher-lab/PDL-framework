...
A further contribution, currently in preparation for Zenodo publication, establishes that the integer exponent~18 appearing in the gravitational coupling $G_{\mathrm{eff}} \sim \varepsilon_G^{18}$ is a topological necessity rather than a counting choice: the elementary \PDL\ closure $K_4$, equipped with its four triangular faces, is homeomorphic to the two-sphere $S^2 = \partial\Delta^3$, and the homology $H_2(S^2;\mathbb{Z}) \cong \mathbb{Z}$ imposes a rank deficit of exactly one at each hierarchical transition of the \PDL\ chain complex, yielding the decomposition $18 = 6+5+4+3$. This result simultaneously identifies the common topological ancestor shared by the \PDL\ exponent~18 and the maximum electron count per period in the periodic table ($2+6+10 = 18$): both are invariants of $S^2$ imposed by the three-dimensionality of physical space (document~D23, \S\ref{subsec:documents}).

Furthermore, a philosophically oriented synthesis, \textit{Whoever We May Be: Existence, Coherence, and Vulnerable Instruments}, revisits the entire PDL corpus from an ontological and existential perspective.\citep{WhoeverWeMayBe2026} It takes as its guiding thread a sentence about human existence as a ``vulnerable instrument'' of a logic of coherence and unfolds its implications in the light of minimal closures, proton architectures, coherence leakage, and emergent metrics.\citep{WhoeverWeMayBe2026} Designed as an accessible entry point for both practising scientists and intellectually engaged non-specialists, it does not introduce new technical results, but articulates how the existing PDL structures can be read as a proposal about what it means to exist in a logically constrained universe.\citep{WhoeverWeMayBe2026}
...
\begin{longtable}{@{}llL{4cm}p{9.0cm}@{}}
...
D20 & 
2026 &
\emph{Universal Coherence Leakage: A Structural Bridge between the Gravitational and Electromagnetic Constants in the PDL Framework} (V3) 
& Derives the structural bridge formula $\Delta r_{\mathrm{val}} = \varepsilon_G \cdot (R_{\mathrm{tot}} R_{\mathrm{surf}}/r_{\mathrm{val}}^2) \cdot (n_u^2-1)/n_u^2$ from three independent geometric principles (hierarchical amplification, golden-ratio surface projection, no-self-loop coherence constraint),corrects two numerical errors in the original internal document, and derives the parameter-free prediction $G_{\mathrm{PDL}} = 6.67448\times10^{-11}~\mathrm{m^3\,kg^{-1}\,s^{-2}}$ ($+27~\mathrm{ppm}$ vs CODATA 2022, within experimental uncertainty). \textbf{Current version: V3} (V1 superseded; V1 contained errors $\varepsilon_G = 0.0073891$, $\Delta r_{\mathrm{val}} = 0.049751$, bridge formula $3\varphi^2\varepsilon_G$ — all corrected in V2/V3). \citep{Universal_Coherence_Leakage_V3_2026} \ref{sec:constants}, \ref{sec:global_map}, \ref{subsec:roadmap_foundational}.\\\\[0.4em]

D21 & 2026 & \textit{Whoever We May Be: Existence, Coherence, and Vulnerable Instruments -- A Philosophical Synthesis of the PDL Framework} &
Philosophical and ontological synthesis of the PDL framework. Connects minimal closures, proton architectures, coherence leakage, and emergent metrics to questions of causality, freedom, transcendence, and human vulnerability, offering an accessible entry point to the programme without adding new technical results.\citep{WhoeverWeMayBe2026} \\\\[0.5em]

D22 & 2026 & \textit{Nuclear Stability and the Periodic Table as Combinatorial Closure Hierarchies in the Projective Dynamic Logo
Framework} &
First formal derivation of the neutron integer quintuplet $(24,28,1032,9960,10992)$ via the exact relation $R_{\mathrm{tot}}(n) = R_{\mathrm{tot}}(p) - (\Delta n+1)^2$; nuclear binding formalised as a balanced exchange of mixed coherent triangles between proton surface and neutron sea; maximum stable neutron number $N_{\mathrm{crit,max}} = 126.1$ derived from a sea-exhaustion condition without free parameters; saturation threshold $Z_{\mathrm{sat}} \approx 20$ (calcium) identified as structural origin of the symmetric/neutron-rich frontier; nuclear spin-orbit coupling traced to the valence-core asymmetry $\Delta n = n_d - n_u = 4$; periodic table interpreted as a map of collective closures bounded by sea exhaustion.
\citep{Nuclear_Stability_PDL_2026} \\[0.4em]

D23 & 2026 & \textit{Topological Origin of the Exponent~18 in the PDL Gravitational Coupling: $K_4$, $S^2$, and the Periodic Table} &
Proves that the exponent~18 in $G_{\mathrm{eff}} \sim \varepsilon_G^{18}$ is a topological necessity, not a counting choice. The elementary \PDL\ closure $K_4$, equipped with its four triangular faces, is homeomorphic to $S^2 = \partial\Delta^3$; the homology $H_2(S^2;\mathbb{Z}) \cong \mathbb{Z}$ imposes a rank deficit of exactly one at each hierarchical transition, yielding the exact decomposition $18 = 6+5+4+3$. The kernel of the proton-to-nuclear transition map is proved analytically over $\mathbb{Q}(\sqrt{5})$ to be $\ker(d_1) = \langle e_{r_{\mathrm{val}}(p)}\rangle$: the valence coherence $r_{\mathrm{val}}(p) = 930$ does not propagate to the nuclear level. Identifies the common topological ancestor of this exponent and the maximum electron count per period in the periodic table ($2+6+10 = 18$): both are invariants of $S^2$ imposed by the three-dimensionality of physical space. All results proved without free parameters over $\mathbb{Q}(\sqrt{5})$. \citep{Topology_18_PDL_2026} \\[0.4em]
...
The coherence–ratio interpretation of the fine-structure constant can be refined by incorporating the primary leakage parameter $\varepsilon$ inferred from the gravitational sector.\cite{Universal_Coherence_Leakage_V3_2026}
...
$\Delta r_{\text{val}}$ in the effective valence count, such that $r_{\text{val,eff}} = 930 + \Delta r_{\text{val}}$.\cite{Alpha_PDL_v2_2026,Universal_Coherence_Leakage_V3_2026}
...
\begin{longtable}{@{}L{1.5cm}L{7.3cm}L{4.2cm}p{1.8cm}@{}}
  ...
  &
\cite{Alpha_PDL_v2_2026,Universal_Coherence_Leakage_V3_2026} \\\\[0.4em]
  ...
  & \cite{Exponent_18_PDL_2026,Topological_Reformulation_PDL_2026,Universal_Coherence_Leakage_V3_2026} \\\\[0.4em]
...
\]
\cite{Exponent_18_PDL_2026,Universal_Coherence_Leakage_V3_2026} 
This leakage should not be interpreted as an uncontrolled stochastic loss, but rather as a systematically structured export of coherence constraints into the surrounding network of closures, which is tightly linked to the ontological characterisation of existence as a *pulsating closure*. Within the broader PDL framework, the quantitative determination of $\varepsilon_G$ from gravitational datasets, as well as its coupling to the electromagnetic sector via the surplus term $\Delta r_{\mathrm{val}}$, are presented in detail in document D20. This document further develops and refines the leakage mechanism that was originally introduced in D17.\citep{Coherence_Leakage_18_2026,Universal_Coherence_Leakage_V3_2026}
...
% ============================================================
% PDL_dependency_diagram_v9.tex
% Replacement for fig:pdl_dependency in PDL_Global_Mapping...v9.tex
% Session 3 — March 2026
%
% Changes vs original diagram:
%  (1) Constants split: α/ħ/k_B (direct) vs G_PDL (via bridge D20)
%  (2) Bridge formula node (D20) explicit, with ε_G and Δr_val as inputs
%  (3) Born's rule / Gleason-type node added (established theorem)
%  (4) Programme/sketch nodes drawn with dashed border + grey fill
%  (5) Legend added
%
% Required packages (already in D_Map preamble):
%   \usepackage{tikz}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    % --- node styles ---
    box/.style={
      draw, solid, rectangle, rounded corners=5pt,
      align=center, minimum width=3.3cm, minimum height=1.0cm,
      fill=white, font=\small
    },
    prog/.style={
      draw, dashed, thick, rectangle, rounded corners=5pt,
      align=center, minimum width=3.3cm, minimum height=1.0cm,
      fill=gray!9, font=\small
    },
    % --- arrow style ---
    >=stealth, thick,
  ...
  %% Column A (x=0):   axioms, TO, ontology
  %% Column B (x=5):   electron, gleason, dynamics, cavity*, ed*
  %% Column C (x=10):  proton, eps
  %% Column D (x=14):  direct constants
  %% Bridge (x=12):    bridge node
  %% Metric (x=12):    metric (lower)
  %% Hydrogen (x=8.5): hydrogen* (lowest, centred)
  %% ============================================================

  % --- Column A: axiomatic + interface ---
  \node[box] (axioms)   at ( 0,    0)   {Axioms \\ \& $4$--$6$ block};
  \node[box] (TO)       at ( 2,   -4.5) {TO interface \\ \& modal constraints};
  \node[box] (ontology) at ( 0,   -7.0) {Ontological \\ layer \\ {\footnotesize(3 documents)}};

  % --- Column B: electron chain ---
  \node[box]  (electron) at ( 4.5,    0)   {Electron \\ $(4$--$6$ realisation)};
  \node[box]  (gleason)  at ( 4.5,   -1.5) {Born's rule \\ {\footnotesize(Gleason-type)}};
  \node[box]  (dynamics) at ( 4.5,   -3.0) {Discrete dynamics \\ \& Schr\"odinger-type};
  \node[prog] (cavity)   at ( 4.5,   -6.5) {Cavity modes \\ \& density of states};
  \node[prog] (ed)       at ( 4.5,  -9.0) {Dirac / \\ Einstein--Dirac};

  % --- Column C: proton chain ---
  \node[box] (proton)   at (9,    0)   {Proton \\ architecture};
  \node[box] (eps)      at (10,   -3) {Primary leakage \\ $\varepsilon_G$};
  % --- Nuclear scale (D22) ---
  \node[box] (nuclear)  at (7.5,   -4.5) {Nuclear scale \\ {\footnotesize(D22)}};

  % --- Column D: direct constants ---
  \node[box] (emcst)    at (12.5, -1.5) {$\alpha,\;\hbar,\;k_B$ \\ {\footnotesize(direct from proton)}};

  % --- Bridge + Metric: between C and D ---
  \node[box] (bridge)   at (11.5, -6) {
    Bridge formula (D20) \\[2pt]
    $\varepsilon_G \;\leftrightarrow\; \Delta r_{\mathrm{val}}$ \\[1pt]
    $\Rightarrow\; G_{\mathrm{PDL}}$
  };
  \node[box] (metric)   at (11.5, -8) {Emergent metric \\ \& gravitation};

  % --- Hydrogen: centred low ---
  \node[prog] (hydrogen) at ( 7.0, -11.5) {Hydrogen spectrum \\ \& redshift};

  %% ============================================================
  %% ARROWS
  %% ============================================================

  % --- Axiomatic chain (top row) ---
  \draw[->] (axioms)   -- (electron);
  \draw[->] (electron) -- (proton);

  % --- Born's rule / Gleason ---
  \draw[->] (electron) -- (gleason);
  \draw[->] (gleason)  -- (dynamics);

  % --- Proton -> eps and emcst ---
  \draw[->] (proton) -- (eps);
  \draw[->] (proton) to[out=0,in=90] (emcst);

  % --- eps -> bridge ---
  \draw[->] (eps) to[out=300,in=90] (bridge);

  % --- proton -> nuclear -> metric ---
  \draw[->] (proton.south) to[out=270,in=90] (nuclear.north);
  \draw[->] (nuclear) to[out=270,in=170] (metric);
  \draw[->] (nuclear) to[out=290,in=180] (bridge.west);

  % --- emcst -> bridge (labelled: via Δr_val) ---
  \draw[->] (emcst.south)
      to[out=270,in=60]
      node[right, yshift=0pt, font=\footnotesize]{via $\Delta r_{\mathrm{val}}$}
      (bridge.north);

  % --- bridge -> metric ---
  \draw[->] (bridge) -- (metric);

  % --- proton -> dynamics (proton constrains dynamical sector) ---
  \draw[->] (proton.south west)
      to[out=215,in=20]
      (dynamics.north east);

  % --- dynamics chain (programme nodes) ---
  \draw[->] (dynamics) -- (cavity);
  \draw[->] (cavity)   -- (ed);

  % --- metric -> ed (emergent metric needed for E-D scheme) ---
  \draw[->] (metric.south)
      to[out=250,in=355]
      (ed.east);
  ...
  % --- left chain: TO and ontology ---
  \draw[->] (axioms) -- (TO);
  \draw[->] (TO)     -- (ontology);

  % --- metric -> TO (metric feeds interface with TO) ---
  \draw[->] (metric.west)
      to[out=180,in=0]
      (TO.east);

  % --- metric -> ontology ---
  \draw[->] (metric.south west)
      to[out=210,in=0]
      (ontology.east);

  % --- axioms -> ontology (direct foundational link) ---
  \draw[->] (axioms.south)
      to[out=270,in=90]
      (ontology.north);
  ...
  %% LEGEND (bottom right)
  %% ============================================================
  \begin{scope}[shift={(-1.0,-11.5)}]
    \node[box,  minimum width=2.2cm, minimum height=0.7cm,
          font=\footnotesize] (leg1) at (0,0)   {Established};
    \node[prog, minimum width=2.2cm, minimum height=0.7cm,
          font=\footnotesize] (leg2) at (3.0,0) {Programme / sketch};
    \node[font=\footnotesize] at (1.5,-0.7) {Epistemic status};
  \end{scope}

  \end{tikzpicture}

  \caption{Schematic dependency structure of the PDL programme (revised, Session~3). \textbf{Solid nodes}: established results. \textbf{Dashed nodes}: components with programme or sketch status. The constants sector is split into two branches: $\alpha$, $\hbar$, $k_B$ are derived directly from the proton surface architecture (documents D$\alpha$, D12, D1), while $G_{\mathrm{PDL}}$ emerges from the structural bridge formula (D20-V3) that equates the primary leakage parameter $\varepsilon_G$ to the electromagnetic valence surplus $\Delta r_{\mathrm{val}}$. The Born's rule node reflects the Gleason-type uniqueness theorem established in document D\_Gleason. The primary leakage parameter $\varepsilon_G$ mediates between the proton architecture and the gravitational sector, and feeds into the emergent metric and dynamical layers.}
...
\begin{longtable}{L{0.8cm}L{1.9cm}L{5.9cm}L{3cm}L{2.2cm}}
...
Interpretation of macroscopic gravitational coupling as hierarchically filtered coherence leakage from nucleonic structures; discrete exponent $18$ applied to $\varepsilon_G$ combined with $\hbar$, $c$, and $m_p$ to reproduce Newton's $G$. The exponent $18$ has been proved to be a topological necessity: $K_4 \cong \partial\Delta^3 \cong S^2$ and $\dim H_2(S^2;\mathbb{Z}) = 1$ impose the exact decomposition $18 = 6+5+4+3$ via the \PDL\ chain complex; moreover, $\ker(d_1) = \langle e_{r_{\mathrm{val}}(p)}\rangle$ is proved analytically over $\mathbb{Q}(\sqrt{5})$ (document~D23). &
Exponent~18: structurally proved theorem (D23, exact arithmetic). Functional form fixed; numerical value of $G$ requires $\varepsilon_G$ calibrated from experiment. &
\citep{Exponent_18_PDL_2026,Coherence_Leakage_18_2026,PDL_Main_2026,Topology_18_PDL_2026,PDL_position} \\\\[0.4em]
...
\citep{Exponent_18_PDL_2026,Alpha_PDL_v2_2026,Universal_Coherence_Leakage_V3_2026} \\\\[0.4em]
...
NUC1 &
Nuclear scale: neutron architecture &
Derivation of the neutron integer quintuplet $(24,28,1032,9960,10992)$ from the proton integers via $R_{\mathrm{tot}}(n) = R_{\mathrm{tot}}(p) - (\Delta n+1)^2$; decomposition of the mass gap $6\mu_n - R_{\mathrm{tot}}(n) = 40$ into structural (25) and dynamical ($\approx 15$) contributions; new relation $R_{\mathrm{tot}}(p) \approx 6\mu_p$at $0.001\%$. &
Structurally established result (exact, no free parameters). & \citep{Nuclear_Stability_PDL_2026} \\\\[0.4em]

NUC2 &
Nuclear scale: stability and magic numbers &
Nuclear binding as balanced exchange of mixed coherent triangles ($T \approx 25$, no free parameter); saturation threshold $Z_{\mathrm{sat}} \approx 20$; maximum magic number $N_{\mathrm{crit,max}} = 126.1$ from sea-exhaustion condition; origin of nuclear spin-orbit coupling in $\Delta n = 4$ (conjecture); full magic number sequence $2,8,20,28,50,82,126$ recovered(structurally motivated conjecture). & 
Partly established (neutron quintuplet, $N_{\mathrm{crit}}$, $Z_{\mathrm{sat}}$); partly conjectural (shell splitting, intermediate magic numbers). & \citep{Nuclear_Stability_PDL_2026} \\[0.4em]

TOP1 &
Topological origin of exponent~18 &
Proof that the exponent~18 in $G_{\mathrm{eff}} \sim \varepsilon_G^{18}$ is a topological necessity imposed by $H_2(S^2;\mathbb{Z}) \cong \mathbb{Z}$, via the identification $K_4 \cong \partial\Delta^3 \cong S^2$. Exact decomposition $18 = 6+5+4+3$ proved over $\mathbb{Q}(\sqrt{5})$. Kernel of proton-to-nuclear transition map identified analytically as $\ker(d_1) = \langle e_{r_{\mathrm{val}}(p)}\rangle$. Common topological ancestor of the \PDL\ exponent and the maximum electron count per period of the periodic table ($2+6+10=18$) identified as the three-dimensionality of physical space. &
\textbf{Structurally proved theorem} (exact symbolic arithmetic, no free parameters). &
\citep{Topology_18_PDL_2026} \\[0.4em]
...
A third foundational goal was to obtain a purely combinatorial derivation of the leakage parameter $\varepsilon$ and the exponent $18$ from intrinsic graph-theoretic invariants of hierarchical closures. This goal has been substantially achieved for the exponent: document~D23 proves that $18$ is a topological invariant of $S^2 = \partial\Delta^3$, imposed by $\dim H_2(S^2;\mathbb{Z}) = 1$ and the identification $K_4 \cong S^2$, yielding the exact decomposition $18 = 6+5+4+3$ over $\mathbb{Q}(\sqrt{5})$ without any free parameter \citep{Topology_18_PDL_2026}. The remaining open task is the derivation of the numerical value of $\varepsilon_G$ itself from first principles, without calibration against the empirical value of $G$; this would promote gravitation from a partially fitted phenomenon to a fully predicted emergent coupling \citep{Exponent_18_PDL_2026,Coherence_Leakage_18_2026,PDL_position}.
\paragraph{(R\_Leakage) Primary leakage parameter and $G$--$\alpha$ bridge.}

Beyond the generic notion of logical leakage $\varepsilon$ introduced in earlier work, the recent analysis of universal coherence leakage identifies a primary, proton-level leakage parameter $\varepsilon_G$ as the central quantity of interest.\citep{Universal_Coherence_Leakage_V3_2026}
...
\[
\varepsilon_G \approx 0.0073891.\citep{Coherence_Leakage_18_2026,Universal_Coherence_Leakage_V3_2026}
\]
...
\begin{enumerate}[label=(\roman*), leftmargin=*]
  ...
  \item \emph{Uniqueness and robustness of the $G$--$\alpha$ bridge.} Delineate the admissible range of combinatorial parameters within which the structurally derived bridge relation
  \begin{equation}
  \Delta r_{\mathrm{val}} \;=\; \varepsilon_G \cdot \frac{R_{\mathrm{tot}}\cdot R_{\mathrm{surf}}}{r_{\mathrm{val}}^2} \cdot \frac{n_u^2-1}{n_u^2}
  \label{eq:bridge_corrected}
  \end{equation}
  remains exact to available precision under controlled perturbations of the proton integer tuple $(24,28,930,10087,11017)$. Here $R_{\mathrm{surf}} = \varphi\,r_{\mathrm{val}}/3$ and the three factors encode respectively: hierarchical amplification ($R_{\mathrm{tot}}/r_{\mathrm{val}} \approx 11.85$), golden-ratio surface projection ($R_{\mathrm{surf}}/r_{\mathrm{val}} = \varphi/3$), and the no-self-loop coherence correction on the up-core ($(n_u^2-1)/n_u^2 = 575/576$). Equation~\eqref{eq:bridge_corrected} is derived from first principles in~\citep{Universal_Coherence_Leakage_V3_2026} and reproduces the empirical ratio $\Delta r_{\mathrm{val}}/\varepsilon_G = 6.3781$ to $0.004\%$, predicting $G_{\mathrm{PDL}} = 6.67448\times10^{-11}$ $\mathrm{m^3\,kg^{-1}\,s^{-2}}$ ($+27~\mathrm{ppm}$ vs CODATA 2022, within experimental uncertainty). 
  
  \textbf{Note:} The formula $\Delta r_{\mathrm{val}} \approx 3\varphi^2\varepsilon_G$ appearing in earlier internal documents has no derivation from the PDL architectural integers and is superseded by equation~\eqref{eq:bridge_corrected}. It should not be cited as a PDL result.
  
  In addition, investigate whether alternative PDL-compatible architectures exist that simultaneously reproduce $G$ and $\alpha$ while violating this bridge relation, thereby systematically probing the uniqueness and structural stability of the proposed $G$--$\alpha$ correspondence.
...
\medskip\noindent\textbf{Nuclear scale (Block~II) --- open problems (D22).} Following the partial implementation of Block~II filters in Ref.~\citep{Nuclear_Stability_PDL_2026}, the following questions remain open within the \PDL\ programme:
\begin{itemize}
  \item \textbf{Shell splitting from $\Delta n$.} Derive analytically the mechanism by which the valence-core asymmetry $\Delta n = n_d - n_u = 4$ generates a spin-orbit splitting of magnitude $\Delta n/(2n_u) \approx 0.083\,\hbar\omega_0$, thereby producing the sub-shell descent responsible for the intermediate magic numbers $20, 28, 50, 82$.
  \item \textbf{Isotopic window width.} Derive the width $\Delta N(Z)$ of the stable isotope window from the nuclear coordination number $c(Z,N)$.
  \item \textbf{Lacunae at $Z = 43$ and $Z = 61$.} Prove that no stable collective surface closure exists for these proton numbers, regardless of neutron count, as a theorem within the \PDL\ coherence framework.
\end{itemize}
...
\begin{longtable}{@{}p{0.9cm}L{5cm}L{6.5cm}L{2.4cm}@{}}
 ...
 &
  ...
  D20 &
  {Universal Coherence Leakage: A Structural Bridge between the Gravitational and Electromagnetic Constants in the PDL Framework} (V3) &
  ...
  D21 & 
  Whoever We May Be: Existence, Coherence, and Vulnerable Instruments -- A Philosophical Synthesis of the PDL Framework & 
  Philosophical and ontological synthesis of the PDL framework.\citep{WhoeverWeMayBe2026} Connects minimal closures, proton architectures, coherence leakage, and emergent metrics to questions of causality, freedom, transcendence, and human vulnerability, and is used here as an interpretative companion to the structural and epistemic mappings. & 
  \ref{sec:purpose_scope}, \ref{sec:overview}, \ref{sec:roadmap} \\\\[0.4em]

D22 &
  Nuclear Stability and the Periodic Table as Combinatorial Closure Hierarchies in the Projective Dynamic Logo Framework &
  First formal derivation of the neutron integer quintuplet $(24,28,1032,9960,10992)$; nuclear binding mechanism; maximum magic number $N_{\mathrm{crit,max}} = 126.1$; saturation threshold $Z_{\mathrm{sat}} \approx 20$; structural origin of nuclear spin-orbit coupling &
  \ref{sec:proton_architecture}, \ref{sec:global_map}, \ref{sec:roadmap} \\[0.4em]

D23 &
  Topological Origin of the Exponent~18 in the PDL Gravitational Coupling: $K_4$, $S^2$, and the Periodic Table &
  Topological proof that the exponent~18 is a necessity: $K_4 \cong \partial\Delta^3 \cong S^2$, $\dim H_2(S^2;\mathbb{Z})=1$; decomposition $18=6+5+4+3$; $\ker(d_1) = \langle e_{r_{\mathrm{val}}(p)}\rangle$ proved over $\mathbb{Q}(\sqrt{5})$; common ancestor of exponent and periodic table structure &
  \ref{sec:constants}, \ref{sec:topology_metric}, \ref{sec:global_map}, \ref{subsec:roadmap_foundational} \\[0.4em]
...
% ── Entry to add in references.bib ──────────────────────────
% @article{Topology_18_PDL_2026,
%   author  = {Laubscher, C.},
%   title   = {Topological Origin of the Exponent~18 in the {PDL} Gravitational
%              Coupling: $K_4$, $S^2$, and the Periodic Table},
%   journal = {Zenodo},
%   year    = {2026},
%   note    = {Proves that the exponent~18 in $G_{\mathrm{eff}} \sim \varepsilon_G^{18}$
%              is a topological necessity via $K_4 \cong \partial\Delta^3 \cong S^2$
%              and $\dim H_2(S^2;\mathbb{Z})=1$; decomposition $18=6+5+4+3$ proved over
%              $\mathbb{Q}(\sqrt{5})$.},
% }
% ──────────────────────────────────────────────────────────────
