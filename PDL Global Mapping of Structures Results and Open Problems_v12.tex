...
\normalsize Version 12 — A Guide for Continuation of the Programme}
...
The current corpus (D01--D39, plus auxiliary documents) has reached the following state:
\begin{itemize}
...
\item Open Problem~OP1 (D36) has been partially resolved: D39 establishes $\kappa = (\varphi/3)\cdot(\rval/\Rtot)$ under a single named hypothesis H3 (the Indifference Lemma), which is the unique remaining residual.
\end{itemize}
The sole foundational open problem is now the derivation of H3 from axioms C1--C4 alone (OP1 of D39, which supersedes OP1 of D36).
\end{abstract}
...
First, it provides a complete and current inventory of the PDL corpus (D01--D39 plus auxiliary documents), with concise descriptions of each document's role and main result.
...
\subsection{What makes this version different from v11}
\label{subsec:diff_v11}

Version~11 of the mapping document covered the corpus up to D36 and identified the algebraic derivation of $\kappa = \Rsurf/\Rtot$ as the sole remaining foundational open problem.
The present version (v12) incorporates three additional documents (D37--D39), updates the open-problem list accordingly, and adds a new section on black hole thermodynamics.
The key advances incorporated are as follows.

\begin{itemize}
\item D37 proves unconditionally that the entropy of a PDL closure is carried entirely by its active surface $\Rsurf$ (Proposition~BH-1), making the Bekenstein--Hawking area law a structural theorem of the four PDL axioms.
\item D38 derives the Bekenstein--Hawking formula $\SBH/k_B = 4\pi(\Meff/\MPl)^2$ algebraically from Gate~3, with three falsifiable parameter-free predictions for primordial black holes and AGN populations.
\item D39 partially resolves OP1 (D36): the two-factor decomposition $\kappa = (\varphi/3)\cdot(\rval/\Rtot)$ is established under a single new named hypothesis H3 (the Indifference Lemma), which is the unique remaining residual of OP1.
\end{itemize}

The reader familiar with v11 should note that: (i) the sole open foundational problem is now the derivation of H3 from C1--C4, replacing the earlier statement of OP1; (ii) the falsifiable prediction table has been extended with three new black-hole entries; and (iii) the epistemic status of H1 has been refined — it now follows from H3 and D05, which isolates H3 as the irreducible assumption.
...
DM & \textit{PDL Global Mapping v10}~\citep{D_Map_v10} & Navigation document for corpus up to D23; superseded by the present v12. \\[0.3em]
...
The most important open problem is OP1 (derivation of H3 from axioms C1--C4); see Open Problem~\ref{op:H3}.
...
This is the unique measure compatible with the $S_4$-symmetry of $\Kfour$ (proved in D36).
\end{hypothesis}

Under H3 alone, H1 follows as a theorem (D39): the two-factor decomposition $\kappa = (\varphi/3)\cdot(\rval/\Rtot) = \Rsurf/\Rtot$ holds, and H2 has been independently absorbed into the Gate~3 proof (D36).
H3 is therefore the unique residual hypothesis, and the derivation of H3 from C1--C4 is the programme's current top open problem (Open Problem~\ref{op:H3}).

\begin{theorem}[Gate~3 — \citep{D36}]
\label{thm:Gate3}
Assume H1 (equivalently, under D39, assume H3).
...
OP1 of D36 is therefore partially resolved, with H3 as the unique residual assumption.
The new open problem is the derivation of H3 from C1--C4 (Open Problem~\ref{op:H3}).
\end{remark}
...
\textcolor{openproblemred}{Open}: $C_{\mathrm{coh}}$ explicit form; coefficient $1/4$ from axioms. &
...
\node[cond, below=0.6cm of BH1] (BH2) {$\SBH/k_B=4\pi(\Meff/\MPl)^2$\\D38 (under H3)};
...
\label{op:H3}
\textbf{Algebraic derivation of H3 (the Indifference Lemma).}
Prove that the measure on the $\Rtot$ relations of a PDL closure is uniform, from axioms C1--C4 alone, without invoking any external geometric or phenomenological input.
This supersedes OP1 of D36 (derivation of H1), since D39 shows that H3 implies H1.
\emph{Consequence}: the entire chain from C1--C4 to $G$, the Einstein equation, and the Bekenstein--Hawking formula becomes fully axiomatic, with $\Delta m_{\mathrm{iso}}$ as the unique remaining external parameter.
\emph{Suggested strategy}: exploit the $S_4$-equivariance of $\Kfour$ (established in D36) and show that no axiom C1--C4 introduces a structural asymmetry between the $\Rtot$ relations; the symmetry argument is already partially assembled in D39.
\emph{Documents}: D36 (OP1), D39 (OP1).
\end{openproblem}
...
The programme currently yields six sharply falsifiable, parameter-free predictions.
...
R10 & $G$ & $\Geff(N)=\sigmaN\cdot\GPDL$ (Gate~3) & \textcolor{proofblue}{Theorem (H3)} \\[0.3em]
R11 & H3 & $\kappa=\Rsurf/\Rtot$ from H3 (D39) & \textcolor{proofblue}{Theorem (H3)} \\[0.3em]
R11b & H3 & Derive H3 from C1--C4 & \textcolor{openproblemred}{Open (OP1)} \\[0.3em]
...
R16 & Gravity & Einstein coupling $\kappa_{\PDL}(N)=\sigmaN\cdot\kappa_{\mathrm{Newton}}$ & \textcolor{proofblue}{Theorem (H3)} \\[0.3em]
R17 & Cosmology & $N_{\mathrm{CMB}}=40$ from neutron architecture & \textcolor{proofblue}{Theorem} \\[0.3em]
R18 & Cosmology & Hubble tension resolved at $0.006\%$ & \textcolor{proofblue}{Theorem (H3)} \\[0.3em]
...
R22 & BH thermo. & $\SBH/k_B=4\pi(\Meff/\MPl)^2$ from Gate~3 & \textcolor{proofblue}{Theorem (H3)} \\[0.3em]
R23 & BH thermo. & PBH threshold shift $+11.9\%$ (Fermi-LAT testable) & \textcolor{proofblue}{Theorem (H3)} \\[0.3em]
...
\item[Proving OP1 (H3)] Start with D39 (the Indifference Lemma and partial resolution), then D36 (the $S_4$-symmetry argument), then D24 for the $\kappa$ definition.
The key target is showing that the $S_4$-equivariance of $\Kfour$ forces a uniform measure on $\Rtot$ under C3 and C4 alone.
...
\item[Indifference Lemma (H3)]
The hypothesis that the probability measure on the $\Rtot$ relations of a PDL closure is uniform.
It is consistent with all PDL axioms, is the unique measure compatible with $S_4$-symmetry of $\Kfour$, and is constrained to better than $0.01\%$ by the Hubble tension.
Under H3, the surface-fraction formula $\kappa = \Rsurf/\Rtot$ follows as a theorem (D39).
...
\bibliography{PDL_references_v12}
