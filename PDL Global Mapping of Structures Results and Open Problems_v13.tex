...
\normalsize Version 13 — A Guide for Continuation of the Programme}
...
The current corpus (D01--D40, plus auxiliary documents) has reached the following state:
\begin{itemize}
...
\subsection{What makes this version different from v12}
\label{subsec:diff_v12}

Version~12 of the mapping document covered the corpus up to D39 and identified the derivation of H3 from axioms C1--C4 as the sole remaining foundational open problem.
The present version (v13) incorporates one additional document (D40) and updates the open-problem list and falsifiable predictions accordingly.
The key advance incorporated is as follows.

\begin{itemize}
\item D40 establishes a coherent and largely parameter-free derivation of nuclear stability and the periodic table from the proton and neutron quintuplets alone. The central results are: (i) $\Nmin(Z) = Z$ for all $Z \leq \Zsat = 20$ as an exact theorem of the neutron survival condition; (ii) the conflict saturation $C(Z>20) = 190\,\Tpp$ exactly; (iii) the valley of stability reproduced at $100\%$ accuracy for all 51 elements from $Z=1$ to $Z=82$ by the rule $\Nmin(Z) = Z + \sum r_{\mathrm{exc}}(Z')$ with $r_{\mathrm{exc}} \in \{0,1,2,3\}$; (iv) eight explicit assembly rules governing the sequential construction of nuclei from hydrogen to iron.
\item D40 identifies a structural connection between OP1 of the nuclear chain (analytic derivation of the filling rates $r_{\mathrm{exc}}(Z)$) and the open problem OP12/BH-3 (derivation of the Bekenstein--Hawking coefficient $1/4$ from axioms): both reduce to counting coherent configurations on a collective PDL surface. This connection places the nuclear and black-hole open problems in the same algebraic category.
\end{itemize}

The reader familiar with v12 should note that: (i) D40 adds a new branch to the dependency map at the nuclear level, orthogonal to the black-hole branch; (ii) OP14 (analytic derivation of $r_{\mathrm{exc}}(Z)$) is the new open problem at the nuclear stability level; and (iii) the conjectured equivalence of OP14 and OP12 (BH-3) is the most significant structural observation of this version.
...
\label{subsec:nuclear_bh_connection}
...
\bibliography{PDL_references_v13}
