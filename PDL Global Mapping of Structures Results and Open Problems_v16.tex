...
\normalsize Version 16 --- A Guide for Continuation of the Programme}
...
The current corpus (D01--D43, plus auxiliary documents) has reached the following state:
\begin{itemize}
...
\item D43 constitutes the first systematic numerical verification of the PDL causal chain using Python/SymPy at 50-decimal precision. The document organises 18 filters into three functional groups, maps the $\epsGeomB$ conjecture of D28, and provides reusable Colab code for all core PDL quantities. D43 establishes the computational infrastructure for the programme and defines the outstanding OP-B problem: identifying the algebraic form of $\epsGeomB$ in $\mathbb{Q}(\sqrt{5})$.
\end{itemize}
There is no longer any foundational open problem at the axiomatic level. The remaining open problems (OP2--OP17) concern higher-level derivations, extensions, and experimental confrontations. The most urgent near-term open problem is OP-B (D43): the algebraic identification of the geometric leakage parameter $\epsGeomB$.
\end{abstract}
...
\label{subsec:aim}
This mapping document pursues four objectives. First, it provides a complete and current inventory of the PDL corpus (D01--D43, plus auxiliary documents), with concise descriptions of each document's role and main result. Second, it makes explicit the logical dependency structure of the programme: which results follow from which, what has been proved, and what remains conjectural or open. Third, it classifies all results according to a rigorous epistemic taxonomy, distinguishing proved theorems, conditional theorems whose hypotheses are explicitly named, structurally motivated conjectures, and open problems. Fourth, it identifies the most productive entry points and continuation paths for a colleague wishing to extend the programme. This document is not a replacement for the primary technical documents; it is a navigational and epistemic guide that complements them.

\subsection{What makes this version different from v15}
\label{subsec:diffv15}
Version~15 of the mapping document covered the corpus up to D42 and confirmed that the entire derivation chain from axioms C1--C4 to the Bekenstein--Hawking entropy was fully axiomatic, with no residual foundational open problem. The present version~v16 incorporates one additional document (D43) and establishes the computational infrastructure of the programme.

\begin{itemize}
\item D43 constitutes the first systematic numerical verification of the PDL causal chain using Python/SymPy at 50-decimal precision. The document introduces three functional groups for the 18 leakage filters: Group~I (fundamental constants: $\alpha$, $\mu$, $G$), Group~II (dynamic equations: Schr\"odinger, Dirac, Born, Einstein), and Group~III (thermodynamic sector: area law, Bekenstein--Hawking, Hubble). A fourth meta-group (Group~0) contains the axiomatic layer (uniqueness of $K_4$, exponent~18, quintuplet).
\item D43 maps the $\epsGeomB$ conjecture of D28 numerically: it verifies that $\epsGeomB = \kappa(1 - 2\kappa^2)^{1/18}$ reproduces the PDL--QCD mass-ratio residual at the $10^{-6}$ level, and provides the Colab infrastructure for systematic testing of alternative closed-form candidates in $\mathbb{Q}(\sqrt{5})$.
\item D43 establishes that $\kappa = 310\varphi/11017 \in \mathbb{Q}(\sqrt{5})$ is confirmed numerically at 50-decimal precision, consistent with its status as an unconditional theorem (D42). It further verifies the two-factor decomposition $\kappa = P_1 \cdot P_2$ at machine precision ($< 10^{-48}$).
\item The 18-filter architecture is now operationalised: each filter is implemented as a Python function, and the residuals at every stage of the chain are quantified. This converts D43 into a living verification document for all future PDL computations.
\end{itemize}

The reader familiar with v15 should note that no epistemic status has changed in v16; the upgrade is computational, not foundational. D43 provides the numerical backbone that makes the programme independently verifiable.
...
DM & PDL Global Mapping v15~\cite{DMapv10} & Navigation document for corpus up to D42; superseded by the present v16.\\[0.3em]
...
D43 & Numerical Verification of the PDL Causal Chain: 18-Filter Architecture and Colab Infrastructure~\cite{D43} & First systematic numerical verification of the PDL chain at 50-decimal precision (Python/SymPy). Organises the 18 filters into four functional groups (Group~0: axioms; Group~I: constants; Group~II: dynamics; Group~III: thermodynamics). Maps the $\epsGeomB$ conjecture of D28 numerically; provides reusable Colab code for all core PDL quantities. Establishes the computational infrastructure of the programme and defines the verification protocol for all future documents.\\[0.4em]
\end{longtable}
...
\item \textbf{Understanding black hole thermodynamics.} Read D37 (area law), then D38 (Bekenstein--Hawking and PBH predictions).
\item \textbf{Understanding nuclear stability and the periodic table.} Read D22, D23, D40, and D41.
\item \textbf{The closure of the foundational programme.} Read D39 first for context, then D42 for the full proof of H3. After D42, all results previously conditional on H3 are unconditional theorems.
\item \textbf{Numerical verification and the computational infrastructure.} Read D43. It provides self-contained Colab code for every major result and defines the verification protocol for future documents. It is the recommended starting point for any computational contribution to the programme.
...
\item \textbf{Primordial black hole survival threshold.} The minimum mass for a primordial black hole to survive to the present day is shifted upward by $11.9\%$ relative to the standard GR prediction: $M_{\min}=\sigma(40)^{-2/3}M_{\mathrm{GR}}\approx 1.119\,M_{\mathrm{GR}}$. Testable via Fermi-LAT gamma-ray constraints.
...
% ============================================================
\section{Numerical Verification Infrastructure (D43)}
\label{sec:numerical}

Document D43~\cite{D43} establishes the computational backbone of the programme. It is the first systematic numerical verification of the PDL causal chain at 50-decimal precision (Python/SymPy).

\paragraph{The four functional groups.} The 18 filters are organised as follows:
\begin{itemize}
\item \textbf{Group~0 (Axiomatic layer):} uniqueness of $K_4$, exponent~18, quintuplet selection. No free parameter; verified by exhaustive enumeration.
\item \textbf{Group~I (Fundamental constants):} $\alpha$, $\mu$, $G$. All three derived from the quintuplet and $m_{\mathrm{iso}}$ alone; residuals quantified at every filter stage.
\item \textbf{Group~II (Dynamic equations):} Schr\"odinger, Dirac, Born, Einstein. The AB criterion is implemented as a Python function and applied to all 768 admissible configurations.
\item \textbf{Group~III (Thermodynamic sector):} area law, Bekenstein--Hawking formula, Hubble tension resolution.
\end{itemize}

\paragraph{The $\epsGeomB$ conjecture.} D43 verifies numerically that $\epsGeomB=\kappa(1-2\kappa^2)^{1/18}$ reproduces the PDL--QCD mass-ratio residual at the $10^{-6}$ level, and provides Colab code for systematic testing of alternative closed-form candidates in $\mathbb{Q}(\sqrt{5})$. The conjecture is \textcolor{conjecturegreen}{open}: no algebraic proof has been given.

\paragraph{Verification protocol.} Every future PDL document that introduces a new numerical claim should include a Colab cell in the D43 format: self-contained imports, 50-decimal precision via \texttt{mpmath.mp.dps=50}, and an explicit residual computation. D43 defines this as the standard verification protocol of the programme.
...
\bottomrule
\endfoot
L0 & Computational verification & \textcolor{proofblue}{Infrastructure}: 18 filters implemented at 50-decimal precision; $\epsGeomB$ conjecture mapped; verification protocol defined. & D43\\[0.4em]
...
L7 & Falsifiable predictions & $\mu\approx 1836.152\,670$ testable at $0.001$~ppm; $m_{\mathrm{iso}}\approx 2.532$~MeV vs.\ FLAG; PBH threshold $+11.9\%$ (Fermi-LAT); $H_{0,\mathrm{CMB}}=67.26$~km\,s$^{-1}$\,Mpc$^{-1}$; $B(E2)$ ratio and $N_{\mathrm{comp}}$ ratio in $^{88}$Ru/$^{92}$Pd. & D28--D30, D35, D38, D41\\
\end{longtable}
...
\begin{openproblem}[OP8 --- Closed-form for $\epsGeomB$ in $\mathbb{Q}(\sqrt{5})$]
\label{op:epsGeomB}
Prove or disprove the conjecture of D28 that $\epsGeomB=\kappa(1-2\kappa^2)^{1/18}$ admits a closed-form expression in $\mathbb{Q}(\sqrt{5})$. D43 has verified numerically at 50-decimal precision that the formula reproduces the residual at $10^{-6}$; an algebraic proof or a counterexample (with a corrected formula) would resolve this. The D43 Colab infrastructure provides all necessary tools for systematic search. \textbf{Priority:} high (immediately actionable via D43 code). \textbf{Entry point:} D28, D43.
\end{openproblem}
...
\node[theorem] at ( 9.0, -13.0) (BH)        {BH entropy\\(D38)};
\node[open]    at ( 12.0, -13.0) (OP8)       {OP8\\$\varepsilon^B_G$};

% ── ROW 6 ── Nuclear + HB + OP17  (y=-11.0)
...
\caption{Critical-path dependency map of the PDL programme (v16).
...
\caption{Complete list of falsifiable, parameter-free PDL predictions (v16). All predictions follow from the quintuplet and $m_{\mathrm{iso}}$ alone, with no additional free parameters. Predictions P1--P6 are derived from the analytical chain; P7--P8 follow from Conjecture~$H_B$ (D41).}
...
P5 & PBH survival threshold & $M_{\min}=1.119\,M_{\mathrm{GR}}$ ($+11.9\%$) & Testable via Fermi-LAT gamma-ray constraints on PBH evaporation; current Fermi-LAT bounds compatible.\\[0.4em]
...
\item \textbf{OP7 and OP8 (47~ppm residual and $\epsGeomB$):} Both are immediately actionable using the D43 Colab infrastructure. Start with the Group~I filters in D43, identify the residual at each stage, and test candidate closed-form expressions in $\mathbb{Q}(\sqrt{5})$.
...
\begin{enumerate}
\item \textbf{D44}: Closed-form derivation of $\epsGeomB$ (OP8) or a proof that no such form exists in $\mathbb{Q}(\sqrt{5})$. Uses D43 infrastructure directly.
\item \textbf{D45}: Born's rule Level~2---derivation of the $\mathrm{U}(1)$ phase from C1--C4 (OP4). This would complete the quantum dynamics layer.
\item \textbf{D46}: Explicit form of $C_{\mathrm{coh}}$ (OP5), enabling the first PDL computation of a gravitational wave observable.
\item \textbf{D47}: Sub-shell filling rates from axioms (OP14), which simultaneously resolves OP12 (BH coefficient $\tfrac{1}{4}$).
\end{enumerate}
...
\item[$\epsGeomB$] Geometric correction factor in the PDL mass-ratio formula (D28 conjecture); numerically verified to $10^{-6}$ in D43; algebraic proof open (OP8).
...
\bibliography{DM_v16_references}
