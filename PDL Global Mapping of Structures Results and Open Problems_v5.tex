...
Within this framework, reality is modelled as a network of minimal logical closures defined on finite signed graphs. Vertices represent informational entities, edges encode binary agreement/disagreement relations, and the dynamical evolution of configurations is governed by a coherence-optimisation principle rather than by externally prescribed field equations. Space, time, and other familiar physical quantities are not treated as primitive inputs but as potentially emergent structures, to be reconstructed from the coherence properties of these closures.
...
\begin{longtable}{@{}p{1.5cm}p{6cm}L{4.5cm}p{2.5cm}@{}}
  ...
  \citep{Alpha_PDL_v2_2026,PDL_intro,Born_Golden_v2_2026} \\[0.4em]
  ...
  \citep{PDL_Main_2026,Exponent_18_PDL_2026,Coherence_Leakage_18_2026,PDL_position} \\
...
Within the PDL framework, \emph{leakage} denotes the fact that no hierarchically organised closure can attain a state of perfectly saturated coherence across all levels simultaneously. A minimal and irreducible deficit of closure necessarily persists and can be parameterised by a leakage coefficient $\varepsilon$, which admits an interpretation as a probability-like measure of residual non-closure. This leakage does not correspond to an uncontrolled or stochastic loss but rather to a systematically structured export of coherence constraints into the surrounding network of closures. It is tightly connected to the ontological conception of existence as \emph{pulsating closure}, as developed in the dedicated analysis of PDL’s existential and boundary structure~\citep{Coherence_Leakage_18_2026,PDL_Main_2026,ExistencePulsatingClosure2026}.

Formally, one associates to each composite closure a constraint graph whose vertices represent local coherence conditions and whose edges encode the corresponding mutual compatibility relations. The parameter $\varepsilon$ is then defined as the fraction of violated constraints in an optimised configuration; it quantifies the minimal residual inconsistency that cannot be removed without compromising or destroying the composite closure itself~\citep{Coherence_Leakage_18_2026}. From this vantage point, leakage is not contingent but instead constitutes a structural characteristic of higher-level modes of existence: whenever multiple organisational layers are superposed, a non-zero portion of coherence must be externalised into the environment rather than being fully internalised.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % --- Flèches dessinées sous les nœuds ---
    % On bascule temporairement sur une couche "background"
  ...
  \end{tikzpicture}
  \caption{Schematic dependency structure within the PDL programme, including the ontological meditation on pulsating closure and the cavity-mode sector with its logical density of states.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=2cm,>=stealth,->]
    % Ligne de base : axioms -> 4-6 -> proton -> constants -> gravitation
    ...
    \node (constants)[draw, rectangle, fill=white, below of=proton, text width=4cm, align=center] {Constants \\$\hbar,\alpha,k_B,G_{\text{eff}}$};
    \node (grav)     [draw, rectangle, fill=white, below of=constants] {Emergent gravitation / cosmology};

    % Branche ontologique
    \node (ontology) [draw, rectangle, fill=white, below of=axioms, yshift=-0.5cm, align=center] {Ontology: \\ existence as \\ pulsating closure};

    % Branche dynamique / cavité
    \node (dynamics) [draw, rectangle, fill=white, below of=fourSix, yshift=-0.5cm] {Discrete dynamics};
    \node (cavity)   [draw, rectangle, fill=white, below of=dynamics, yshift=-0.5cm, align=center] {Cavity modes \\ \& logical DOS};
    ...
    \draw (dynamics) -- (cavity);
  \end{tikzpicture}
  \caption{Conceptual dependency chain within PDL, including the ontological reading of pulsating closure and the cavity-mode sector with logical density of states.}
...
Several foundational combinatorial problems are central to the further development of the PDL programme. A first priority concerns the uniqueness of the minimal $4$–$6$ block: establishing a rigorous proof or disproof that no smaller or alternative configuration satisfies the axioms with comparable optimality would resolve whether the $4$–$6$ regime constitutes a strictly necessary primitive, or instead represents just one admissible choice within a broader class of permissible structures \citep{PDL_Main_2026,PDL_position}. A second major objective is the exhaustive classification of admissible proton architectures in a neighbourhood of the integer tuple $(24,28,930,10087,11017)$, with the aim of determining whether the currently preferred proton model is uniquely fixed (up to isomorphism) by the optimisation constraints, or whether there exists a nontrivial family of degenerate alternatives consistent with the same criteria \citep{Combinatorial_Proton_v2_2026,PDL_position}. A third foundational goal is to obtain a fully combinatorial derivation of the leakage parameter $\varepsilon$ and the exponent $18$ from intrinsic graph-theoretic invariants of hierarchical closures, thereby eliminating the present reliance on calibration against the empirical value of $G$ and elevating gravitation from a partially fitted phenomenon to a fully predicted emergent coupling \citep{Exponent_18_PDL_2026,Coherence_Leakage_18_2026,PDL_position}.
