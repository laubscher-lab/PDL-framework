...
\begin{longtable}{@{}L{1.5cm}L{7.3cm}L{4.2cm}p{1.8cm}@{}}
  ...
  &
\cite{Alpha_PDL_v2_2026,Universal_Coherence_Leakage_2026} \\\\[0.4em]
  ...
  & \cite{Exponent_18_PDL_2026,Topological_Reformulation_PDL_2026,Universal_Coherence_Leakage_2026} \\\\[0.4em]
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    node distance=0.8cm,
    box/.style={draw, rectangle, rounded corners=5pt, align=center,
    ...
    \node[box, right=2cm of axioms] (electron) {Electron \\ $(4$--$6$ realisation)};
    \node[box, right=2cm of electron] (proton) {Proton \\ architecture};

    % Colonne proton -> fuite -> constantes -> métrique
    \node[box, below=1cm of proton] (eps) {Primary leakage \\ $\varepsilon_G$};
    \node[box, below=1cm of eps] (constants) {Constants \\ $\hbar,\, \alpha,\, k_B,\, G_{\text{eff}}$};
    \node[box, below=2.4cm of constants] (metric) {Leakage, \\ emergent metric, \\ gravitation};

    % Colonne centrale gauche : dynamique
    \node[box, below=1cm of electron] (dynamics) {Discrete \\ dynamics \\ \& Schr\"odinger-type};

    % Nœud: modes de cavité / densité d'états
    \node[box, below=1cm of dynamics] (cavity) {Cavity modes \\ \& logical \\ density of states};

    % Nœud : Dirac / Einstein–Dirac
    \node[box, below=2cm of cavity, xshift=1.3cm] (einsteindirac) {Dirac / \\ Einstein--Dirac \\ scheme};

    % Nœud de test phénoménologique
    \node[box, below=1cm of einsteindirac] (hydrogen) {Hydrogen \\ spectrum \\ \& redshift};

    % Colonne gauche : interface TO
    \node[box, below=1cm of axioms] (TO) {TO interface \\ \& modal \\ constraints};

    % Nœud ontologique : existence as pulsating closure
    \node[box, below=1cm of TO] (ontology) {Existence as \\ pulsating \\ closure};
