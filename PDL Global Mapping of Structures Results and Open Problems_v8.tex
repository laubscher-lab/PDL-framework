...
% ============================================================
% PDL_dependency_diagram_v2.tex
% Replacement for fig:pdl_dependency in PDL_Global_Mapping...v7.tex
...
\begin{figure}[H]
  ...
  \node[prog] (cavity)   at ( 4.5,   -6.0) {Cavity modes \\ \& density of states};
  \node[prog] (ed)       at ( 4.5,  -8.0) {Dirac / \\ Einstein--Dirac};

  % --- Column C: proton chain ---
  \node[box] (proton)   at (9,    0)   {Proton \\ architecture};
  \node[box] (eps)      at (9,   -1.5) {Primary leakage \\ $\varepsilon_G$};

  % --- Column D: direct constants ---
  \node[box] (emcst)    at (13, -1.5) {$\alpha,\;\hbar,\;k_B$ \\ {\footnotesize(direct from proton)}};

  % --- Bridge + Metric: between C and D ---
  \node[box] (bridge)   at (10.5, -3.5) {
    Bridge formula (D20) \\[2pt]
    $\varepsilon_G \;\leftrightarrow\; \Delta r_{\mathrm{val}}$ \\[1pt]
    $\Rightarrow\; G_{\mathrm{PDL}}$
  };
  \node[box] (metric)   at (11, -5.5) {Emergent metric \\ \& gravitation};

  % --- Hydrogen: centred low ---
  \node[prog] (hydrogen) at ( 8.0, -9.5) {Hydrogen spectrum \\ \& redshift};
  ...
  \draw[->] (emcst.south)
      to[out=270,in=30]
      node[right, yshift=2pt, font=\footnotesize]{via $\Delta r_{\mathrm{val}}$}
      (bridge.north east);
  ...
  \caption{Schematic dependency structure of the PDL programme
    (revised, Session~3).
    \textbf{Solid nodes}: established results.
    \textbf{Dashed nodes}: components with programme or sketch status.
    The constants sector is split into two branches:
    $\alpha$, $\hbar$, $k_B$ are derived directly from the proton surface
    architecture (documents D$\alpha$, D12, D1),
    while $G_{\mathrm{PDL}}$ emerges from the structural bridge formula
    (D20-V3) that equates the primary leakage parameter $\varepsilon_G$
    to the electromagnetic valence surplus $\Delta r_{\mathrm{val}}$.
    The Born's rule node reflects the Gleason-type uniqueness theorem
    established in document D\_Gleason.
    The primary leakage parameter $\varepsilon_G$ mediates between
    the proton architecture and the gravitational sector,
    and feeds into the emergent metric and dynamical layers.}
