...
\normalsize Version 18 --- A Guide for Continuation of the Programme}
...
The current corpus (D01--D48, plus auxiliary documents) has reached the following state:
\begin{itemize}
...
\item The quantum dynamics layer is \textbf{complete}: the Schr\"odinger equation (D32), Dirac equation (D33), Born's rule Level~1 via Gleason uniqueness (D34), the Einstein coupling equation (D35), and Born's rule Level~2 including $\mathrm{U}(1)$ phase freedom (D46) have all been derived from C1--C4 with no free parameter.
...
\item D48 partially resolves OP2-D35: the coherence stress-energy tensor $\Ccoh$ is derived with three unconditional components ($C^{(\mathrm{mass})}$, $C^{(\mathrm{spin})}$, $C^{(\mathrm{orb})}$) and one partially resolved component ($C^{(\mathrm{leak})}$), yielding the PDL Einstein equation with an explicit source. The constant $C$ in $C^{(\mathrm{leak})}$ remains open (OP1-D35).
...
\end{itemize}
There is no longer any foundational open problem at the axiomatic level. The remaining open problems (OP1-D35, OP2--OP17 and three new problems from D48) concern higher-level derivations, extensions, and experimental confrontations.
\end{abstract}
...
\subsection{What makes this version different from v17}
\label{subsec:diffv17}
Version~17 of the mapping document covered the corpus up to D45 and marked the closure of the causal chain C1--C4~$\to$~$G$. The present version~v18 incorporates three additional documents (D46, D47, D48) and marks the completion of two major programme layers.

\begin{itemize}
\item D46 resolves OP4 (Born's rule Level~2). The $\mathrm{U}(1)$ phase freedom $\psi \mapsto e^{i\alpha}\psi$ of quantum mechanics is derived from the $K_4$ pulsation structure via three propositions. Proposition~1 establishes that the global sign equivalence $s \sim -s$ of $K_4$ coherent configurations, combined with $T^2 = -I_2$ (D33), generates a canonical $\mathrm{U}(1)$ action with Hopf fibration $S^1 \hookrightarrow S^3 \to S^2$ as orbit structure. Proposition~2 proves that the $(A)\wedge(B)$ criterion is $\mathrm{U}(1)$-invariant over all 8 coherent configurations. Proposition~3 proves that the unique $\mathrm{U}(1)$-equivariant probability measure on $S^3$ consistent with Level~1 Gleason axioms is Born's rule $P = |\langle\theta|\psi\rangle|^2$. As a consequence, the quantum dynamics layer of the PDL programme is complete: Schr\"odinger (D32) $+$ Dirac (D33) $+$ Born Level~1 (D34) $+$ Born Level~2 $+$ $\mathrm{U}(1)$ (D46).

\item D47 resolves OP13 and OP14 simultaneously. Theorem~3.1 establishes that $\Delta n = 4$ is the unique value in $\{0, 4, 8, \ldots\}$ such that the discriminant of the quasi-completeness equation $3n_u^2 + (2\Delta n - 3)n_u + \Delta n(\Delta n - 1) - 1860 = 0$ is a perfect square ($149^2$), forcing $n_u = 24$, $n_d = 28$, and $s = 1/12$ as unconditional theorems of C1--C4. Lemma~5.2 (Mirror Lemma) proves that the harmonic oscillator PDL levels of the proton and neutron are isomorphic, with identical splitting $1/12$ and identical degeneracies. Theorem~6.5 derives the filling rate $r_{\mathrm{exc}}(Z) = 0$ for $Z \in \{28, 50, 82\}$ (magic closures) and $r_{\mathrm{exc}}(Z) = 1$ otherwise, verified exhaustively for 31 sub-shell boundaries with zero exception and zero free parameter. Corollary~7.1 promotes the valley-of-stability formula for $Z = 1$--82 to an unconditional theorem of C1--C4. The nuclear stability layer of the PDL programme is complete.

\item D48 partially resolves OP2-D35 (explicit form of $\Ccoh$). Theorem~3.1 proves that the coherence volume $V_C = (4/3)\pi[\hbar/(m_p c)]^3$ is doubly forced: by the temporal chain (D10a: $\omega_p \to \lambda_C \to V_C$) and by the geometric chain (D23: $S^2$ topology $\to n=3 \to (4/3)\pi$; $R_{\mathrm{geom}} = \lambda_C$ algebraically exact), with numerical agreement at machine precision ($V_{\mathrm{geom}}/V_C = 1.00000000$, float64). The coherence mass density $\rhocoh(N) = \sigmaN \cdot \Rsurf \cdot m_p / (V_C \cdot \Rtot)$ is derived as an unconditional theorem. The four components of the PDL coherence stress-energy tensor are: $C^{(\mathrm{mass})}_{\mu\nu} = \rhocoh(N)\,c^2\,u_\mu u_\nu$ (unconditional theorem), $C^{(\mathrm{spin})}_{\mu\nu} \sim \hbar^2\,\rhocoh^2/(m_p c^2)$ (theorem, correction $\sim 10^{-12}$ relative to $C^{(\mathrm{mass})}$), $C^{(\mathrm{orb})}_{\mu\nu} = m_p\,\langle j_\mu j_\nu\rangle/\rhocoh$ (theorem, state-dependent via D32), and $C^{(\mathrm{leak})}_{\mu\nu} = -C\,\eta_L^{18}\,(m_p c/\hbar)^2\,g_{\mu\nu}$ (partial: $\eta_L^{18}$ is a theorem from D17 and D30; $C$ remains open as OP1-D35, bounded observationally at $C \approx 8.16\times10^{-46}$ from Planck 2020). Corollary~9.1 gives the PDL Einstein equation with explicit complete source. Three new open problems are identified: OP1-D35 (derive $C$ from C1--C4), OP-pressure (equation of state $w = P/(\rhocoh c^2)$), and OP-London (London equation from $C^{(\mathrm{orb})}$ + D46 + D12).
\end{itemize}

The reader familiar with v17 should note the following epistemic changes: OP4 is resolved (D46, moved to resolved list); OP13 and OP14 are resolved (D47, moved to resolved list); OP2-D35 is partially resolved (D48); three new open problems are introduced (OP1-D35, OP-pressure, OP-London); the quantum dynamics layer is now complete; the nuclear stability layer is now complete; the corpus gains three rows in the inventory table; and the dependency map gains D46, D47, and D48 nodes.
...
DM & PDL Global Mapping v18~\cite{DMapv10} & Navigation document for the complete corpus D01--D48; the present document.\\[0.3em]
...
\caption{Eight-layer epistemic architecture of the PDL programme (updated for v18). Each layer lists the status of its key claims and the primary documents supporting them.}
\label{tab:epistemic}\\
\toprule
...
L4 & Quantum dynamics \textbf{[COMPLETE]} & \textcolor{proofblue}{Theorems}: Schr\"odinger~\cite{D32}; Dirac~\cite{D33}; Born Level~1~\cite{D34}; Born Level~2 + $\mathrm{U}(1)$ (OP4 resolved)~\cite{D46}. & D32, D33, D34, D46\\[0.4em]
L5 & Gravitational and black hole dynamics & \textcolor{proofblue}{Theorem} (unconditional): $G_{\mu\nu}^{\mathrm{eff}}=8\pi G(N)\Ccoh$. \textcolor{proofblue}{Theorem}: area law $S\propto\Rsurf$ unconditional~\cite{D37}. \textcolor{proofblue}{Theorem}: $\SBH/k_B=4\pi(\Meff/\MPl)^2$~\cite{D38}. $\Ccoh$ explicit: $C^{(\mathrm{mass})}$, $C^{(\mathrm{spin})}$, $C^{(\mathrm{orb})}$ theorems; $C$ in $C^{(\mathrm{leak})}$ open (OP1-D35)~\cite{D48}. & D35, D36, D37, D38, D48\\[0.4em]
...
The following list supersedes the open-problem list of v17. Problems are classified by priority and domain. OP1, OP4, OP8, OP13, and OP14 have been fully resolved and are retained for historical reference only.
...
\begin{openproblem}[OP1-D35 --- Derivation of the leak constant $C$ from axioms]
\label{op:leakconstant}
Derive the prefactor $C$ in $C^{(\mathrm{leak})}_{\mu\nu} = -C\,\eta_L^{18}\,(m_p c/\hbar)^2\,g_{\mu\nu}$ from axioms C1--C4. This is the PDL equivalent of the cosmological constant problem: $C$ is bounded observationally at $C \approx 8.16\times10^{-46}$ (from $\Lambda_{\mathrm{obs}} = 1.089\times10^{-52}$~m$^{-2}$, Planck 2020), but its derivation from the axioms remains open. \textbf{Priority:} high. \textbf{Entry point:} D48, D17, D30.
\end{openproblem}
...
\label{op:darkenergy}
Investigate whether the cosmological constant $\Lambda$ can be identified with the saturated leakage $\kappa^{18}$ evaluated at $N\to\infty$, and derive the observed value $\Lambda\approx 1.1\times10^{-52}$~m$^{-2}$ from the quintuplet without additional parameters. \textbf{Priority:} medium. \textbf{Entry point:} D24, D26, D35, D48.
\end{openproblem}

\begin{openproblem}[OP12 --- Derivation of the Bekenstein--Hawking coefficient $\tfrac{1}{4}$ from axioms]
\label{op:BHcoeff}
Derive the coefficient $\tfrac{1}{4}$ in $\SBH=k_Bc^3A/(4\hbar G)$ from the combinatorial structure of $K_4$, without invoking the Schwarzschild geometry as an external input. \textbf{Priority:} high. \textbf{Entry point:} D37, D38.
\end{openproblem}
...
\label{op:pressure}
Derive the equation of state $w = P/(\rhocoh\,c^2)$ of the PDL coherence fluid from the axioms. The mass-dominated component gives $w \approx 0$; corrections from $C^{(\mathrm{spin})}$ and $C^{(\mathrm{orb})}$ may shift $w$ at higher densities. \textbf{Priority:} medium. \textbf{Entry point:} D48.
\end{openproblem}

\begin{openproblem}[OP-London --- London equation from PDL axioms]
\label{op:London}
Derive the London equation $\mathbf{j}_s = -(n_s e^2/m_e c)\mathbf{A}$ from the orbital component $C^{(\mathrm{orb})}_{\mu\nu}$ of the coherence tensor (D48), combined with the $\mathrm{U}(1)$ phase freedom (D46) and the fine-structure constant $\alpha$ (D12). The analogy between the Meissner effect and the $\mathrm{U}(1)$-invariant suppression of phase fluctuations in $K_4$ is conjectured but not yet proved. \textbf{Priority:} medium. \textbf{Entry point:} D46, D48, D12.
\end{openproblem}
...
% ── ROW 7 ── OP1-D35 + OP12 + OP-London
\node[open] at ( 0.0,-18.5) (OP1D35)  {OP1-D35\\leak const. $C$};
\node[open] at ( 12.0,-18.5) (OP12)    {OP12\\$\tfrac{1}{4}$ coeff.};
\node[open] at ( 4.4,-18.5) (OPLondon){OP-London};
...
% Ccoh → OP1-D35, OP-London
\draw[arr] (Ccoh.south)      -- (OP1D35.north);
\draw[arr] (Ccoh.south)      to[out=-90, in=90] (OPLondon.north);
\draw[arr] (BornL2.south)    to[out=-90, in=90] (OPLondon.north);
...
\caption{Critical-path dependency map of the PDL programme (v18). \textcolor{proofblue}{Blue nodes}: unconditional theorems. \textcolor{conjecturegreen}{Green}: conjectures. \textcolor{openproblemred}{Red}: open problems. Grey: unique external input ($m_{\mathrm{iso}}$). The quantum dynamics layer (Schr\"odinger, Dirac, Born L1, Born L2/U(1)) and the nuclear stability layer (OP13, OP14, valley of stability) are complete. The coherence tensor $\Ccoh$ is partially resolved (D48), with OP1-D35 and OP-London as frontier problems.}
...
\caption{Complete list of falsifiable, parameter-free PDL predictions (v18). All predictions follow from the quintuplet and $m_{\mathrm{iso}}$ alone, with no additional free parameters. Predictions P1--P6 are derived from the analytical chain; P7--P8 follow from Conjecture~$H_B$ (D41); P9 is derived from D45.}
...
\item \textbf{OP12 (BH coefficient $\tfrac{1}{4}$):} Requires D37, D38, and the combinatorial structure of D47. The mirror between nuclear filling (D47) and Bekenstein--Hawking counting may be exploited.
\item \textbf{OP-London (London equation from PDL):} Requires D46 (U(1) phase freedom), D48 ($C^{(\mathrm{orb})}$), and D12 ($\alpha$). The analogy with Meissner effect is conjectured and documented in D48; the formal derivation remains open.
\item \textbf{OP17 (FRIB/RIKEN predictions):} Experimental. Contact the PDL team before submission to ensure the prediction is correctly stated and that the appropriate source data will be requested from the experiment. Contact F.\ Recchia and S.\ M.\ Lenzi (University of Padova) regarding the D41 nuclear confrontation context.
\end{itemize}
...
\begin{enumerate}
\item \textbf{D49}: Derivation of the leak constant $C$ from C1--C4 (OP1-D35). This is the PDL analogue of deriving the cosmological constant from first principles.
\item \textbf{D50 (or D49-bis)}: The London equation from $C^{(\mathrm{orb})}$ + D46 + D12 (OP-London). This would establish the first PDL connection to superconductivity.
...
\item \textbf{Journal submission}: D42 (H3 theorem) or D47 (nuclear stability as corollary of axioms) are the strongest candidates for a first peer-reviewed submission.
\end{enumerate}
...
\item[$\Ccoh$] Coherence stress-energy tensor appearing in the PDL Einstein equation; explicit form partially derived in D48: $C^{(\mathrm{mass})}$, $C^{(\mathrm{spin})}$, $C^{(\mathrm{orb})}$ are unconditional theorems; $C^{(\mathrm{leak})}$ is partial (OP1-D35).
...
\item[OP$n$] Open Problem $n$; see Section~\ref{sec:openproblems}. OP1, OP4, OP8, OP13, OP14 fully resolved.
...
\bibliography{DM_v18_references}
