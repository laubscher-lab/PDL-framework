...
\normalsize Version 22 --- A Guide for Continuation of the Programme}
...
The current corpus (D01--D54, plus auxiliary documents and the PDL-V series DL01--DL02) has reached the following state:
\begin{itemize}
...
\end{itemize}
There is no longer any foundational open problem at the axiomatic level. The causal chain from C1--C4 to the cosmological constant $\Lambda_{\mathrm{PDL}}$ is complete. The equation of state of the coherence fluid (D54) is fully resolved. The remaining open problems concern higher-level derivations (electroweak sector, masses of higher generations), extensions (superheavy nuclei, dark matter identification), and experimental confrontations.
\end{abstract}
...
\subsection{What makes this version different from v21}
\label{subsec:diffv21}
Version~21 of the mapping document covered the corpus up to D53 and incorporated the corrections to D48 (now version~3). The present version~v22 incorporates two new results: the resolution of OP-pressure by D54 (equation of state of the coherence fluid), and the opening of the PDL-V programme on life and consciousness (DL series, documents DL01 and DL02). It also adds Part~III of this document, dedicated to the PDL-V programme, clearly separated from the physics of fundamental constants.
...
\node[theorem] at (12.0, -2.5) (Gate2)     {Gate~2\\$a=2$\\(D30)};
...
\node[theorem] at (11.2,-10.0) (BornL2)    {Born L2\\U(1)\\(D46)};

% ── ROW 5 ── Ccoh + Hubble + AreaLaw + BH
\node[theorem] at ( 0.0,-12) (Ccoh)      {$\Ccoh$ explicit\\(D48, partial)};
\node[theorem] at ( 2.8,-12) (Hubble)    {Hubble tension\\$0.006\%$\\(D27/D35)};
\node[theorem] at ( 5.6,-12) (AreaLaw)   {Area law\\(D37)};
\node[theorem] at ( 9.5,-12) (BH)        {BH entropy\\(D38)};
\node[theorem] at (12.5,-12) (D45)       {PBH threshold\\$+11.89\%$\\(D45)};

% ── ROW 6 ── Nuclear + Mirror + D47 + HB
\node[theorem]    at ( 2.0,-14.0) (Nuclear)  {Valley of\\stability\\(D40)};
\node[theorem]    at ( 5.2,-14.0) (D47node)  {OP13+OP14\\$\Delta n=4$, $r_{\mathrm{exc}}$\\(D47)};
\node[conjecture] at ( 8.4,-14.0) (HB)       {$H_B$ conjecture\\(D41)};
\node[open]       at (12.0,-14.0) (OP17)     {OP17\\FRIB/RIKEN};

% ── ROW 7 ── D49 + D50 + D51/D52 + Lambda
\node[theorem] at ( 0.0,-16.5) (D51D52)  {OP1-D35\\$C\to\Lambda$\\(D51+D52)};
\node[theorem] at ( 4.4,-16.5) (D49)     {London eq.\\(D49)};
\node[theorem] at ( 8.0,-16.5) (D50)     {$\tfrac{1}{4}$ coeff.\\(D50)};
\node[theorem] at (12.0,-16.5) (Lambda)  {$\Lambda_{\mathrm{PDL}}$\\complete};
...
\begin{enumerate}
\item \textbf{DM v22}: The present document. Covers D01--D54 and DL01--DL02, maps the complete resolved state of the programme including OP-pressure (D54) and the life/consciousness hierarchy (DL series).
\item \textbf{D-electroweak}: Derivation of the Weinberg angle $\sin^2\theta_W$ from C1--C4. The tree-level value $\sin^2\theta_W = 1/4$ follows from the SU(2) generator counting in $K_4$ (D33, D46); the correction to 0.231 requires an equivalent of the renormalisation group flow, not yet in the corpus.
...
\item \textbf{Journal submission}: D49 (London equation), D50 (Bekenstein--Hawking $\tfrac{1}{4}$), D51+D52 (cosmological constant), D53 (causal closure synthesis), D54 (equation of state), or D42 (H3 theorem) are strong candidates. D53 in particular offers a compact, self-contained account of the complete C1--C4 $\to$ $\Lambda$ chain.
...
\bibliography{DM_v22_references}
