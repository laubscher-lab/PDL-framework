...
\normalsize Version 24 --- A Guide for Continuation of the Programme}
...
The current corpus (D01--D55, plus auxiliary documents and the PDL-V series DL01--DL02) has reached the following state:
\begin{itemize}
...
\subsection{What makes this version different from v22}
\label{subsec:diffv22}
Version~22 covered the corpus up to D54 and DL01--DL02, mapping the resolution of OP-pressure and the opening of the PDL-V programme. The present version~v23 incorporates one new result of major structural significance: document D55 derives the Weinberg angle $\theta_W = 19\pi/119$ as an unconditional theorem of C1--C4, resolving OP10 at the level of $\theta_W$. Together with D33 and D46, the gauge group geometry of the Standard Model electroweak sector is now entirely derived from axioms. A new prediction P10 ($\Delta m_{\mathrm{iso}} = 2.446$~MeV) is added. The reader familiar with v22 should note: D55 added to corpus table; Layer~15 (electroweak) updated from open to partially resolved; OP10 reclassified; continuation guide updated; P10 added to predictions table; bibliography gains two entries (D55, FLAG 2024).
...
\begin{openproblem}[OP10 --- Partially resolved: Weinberg angle (D55); $W/Z$ mass ratio open]
\label{op:weak}
\textcolor{proofblue}{$\theta_W$ resolved by D55~\cite{D55}.} The Weinberg angle is derived as an unconditional theorem of C1--C4: $\theta_W = 19\pi/119$, where $k_2 = 19$ (D51 theorem) and $N_{\mathrm{total}} = 119$ (Lemma~D, proved exhaustively). The predicted $\sin^2\theta_W = 0.231196$ agrees with PDG~2024 at $0.48\,\sigma$. The secondary prediction $\Delta m_{\mathrm{iso}} = 2.446$~MeV (Prediction~P10) is falsifiable by next-generation lattice QCD. Remaining open: (OP10-c) derive the $W/Z$ mass ratio $M_W/M_Z = \cos(19\pi/119)$ as a parameter-free prediction; (OP10-d) experimental test of $\Delta m_{\mathrm{iso}}$ at $\pm 0.04$~MeV precision. \textbf{Priority:} medium. \textbf{Entry point:} D33, D46, D51, D55.
\end{openproblem}
...
\node[theorem] at ( 0.0, -3.0) (quintuplet){Quintuplet\\$(24,28,930,$\\$10087,11017)$};
\node[theorem] at ( 4.5, -2.5) (kappa)     {$\kappa\in\mathbb{Q}(\sqrt{5})$\\(D42, unconditional)};

% ── ROW 2 ── Gate1 + Gate2
\node[theorem] at ( 8.5, -3.5) (Gate1)     {Gate~1\\$155/11017$\\(D29)};
\node[theorem] at (12.0, -2.0) (Gate2)     {Gate~2\\$a=2$\\(D30)};

% ── ROW 3 ── miso + G + Gate3 + mu
\node[ext]     at ( 0.0, -5.0) (miso)      {$m_{\mathrm{iso}}$\\(ext., QCD)};
\node[theorem] at ( 0.0, -6.5) (G)         {$G$ theorem\\(D21/D25)};
\node[theorem] at ( 4.5, -5.5) (Gate3)     {Gate~3\\$G_{\mathrm{eff}}=\sigma(N)\!\cdot\!G_{\mathrm{PDL}}$\\(D36/D42)};
\node[theorem] at (12.0, -6.0) (mu)        {$\mu$ prediction\\$1836.152\,670$\\(D28--D30)};

% ── ROW 4 ── Einstein + Schrod + Dirac + Born L1 + Born L2
\node[theorem] at ( 0.0,-9.0) (Einstein)  {Einstein eq.\\(D35)};
\node[theorem] at ( 2.8,-9.0) (Schrod)    {Schr\"odinger\\(D32)};
\node[theorem] at ( 5.6,-9.0) (Dirac)     {Dirac\\(D33)};
\node[theorem] at ( 8.4,-9.0) (BornL1)    {Born L1\\(D34)};
\node[theorem] at (11.2,-9.0) (BornL2)   {Born L2\\U(1)\\(D46)};

% ── ROW 4b ── Electroweak: k2 + LemmaD + Weinberg (D55) + WZ open
\node[theorem] at ( 3.5,-12.0) (k2node)   {$k_2=19$\\(D51)};
\node[theorem] at ( 7.0,-12.0) (LemmaD)   {Lemma~D\\$N_{\mathrm{tot}}=119$\\(D55)};
\node[theorem] at (10.5,-12.0) (D55node)  {$\theta_W=19\pi/119$\\$\sin^2\theta_W=0.2312$\\(D55)};
\node[open]    at (14.0,-12.0) (WZopen)   {OP10-c\\$W/Z$ ratio\\open};

% ── ROW 5 ── Ccoh + Hubble + AreaLaw + BH
\node[theorem] at ( 0.0,-14.1) (Ccoh)     {$\Ccoh$ explicit\\(D48)};
\node[theorem] at ( 2.8,-14.1) (Hubble)   {Hubble tension\\$0.006\%$\\(D27/D35)};
\node[theorem] at ( 5.6,-14.1) (AreaLaw)  {Area law\\(D37)};
\node[theorem] at ( 9.5,-14.1) (BH)       {BH entropy\\(D38)};
\node[theorem] at (12.5,-14.1) (D45)      {PBH threshold\\$+11.89\%$\\(D45)};

% ── ROW 6 ── Nuclear + D47 + HB + OP17
\node[theorem]    at ( 4.0,-16.3) (Nuclear) {Valley of\\stability\\(D40)};
\node[theorem]    at ( 7.2,-16.3) (D47node) {OP13+OP14\\$\Delta n=4$, $r_{\mathrm{exc}}$\\(D47)};
\node[conjecture] at ( 10.5,-16.3) (HB)      {$H_B$ conjecture\\(D41)};
\node[open]       at (14.0,-16.3) (OP17)    {OP17\\FRIB/RIKEN};

% ── ROW 7 ── D49 + D50 + D51/D52 + Lambda
\node[theorem] at ( 0.0,-19.1) (D51D52)  {OP1-D35\\$C\to\Lambda$\\(D51+D52)};
\node[theorem] at ( 5.5,-18.3) (D49)     {London eq.\\(D49)};
\node[theorem] at ( 8.5,-18.3) (D50)     {$\tfrac{1}{4}$ coeff.\\(D50)};
\node[theorem] at (14.0,-19.1) (Lambda)  {$\Lambda_{\mathrm{PDL}}$\\complete};
...
\draw[arr] (Dirac.south)     to[out=-90, in=90] (k2node.north);
\draw[arr] (BornL2.south)    to[out=-90, in=90] (LemmaD.north);
\draw[arr] (k2node.east)     -- (LemmaD.west);
...
\begin{enumerate}
\item \textbf{DM v24}: The present document. Incorporates D56 (Theorem~$N_{\mathrm{comp}}(k)=k$, OP-D41-1-A resolved) and the new Open Problem OP-E2-PDL.
\item \textbf{OP-E2-PDL}: Identification of the $E2$ operator in the PDL formalism (entry points: D29, D32, D41, D56). Resolving this would elevate Conjecture~$H_B$ to a theorem.
\item \textbf{D-electroweak-WZ}: Derivation of the $W/Z$ mass ratio $M_W/M_Z = \cos(19\pi/119)$ as a parameter-free prediction (OP10-c). Entry point: D55.
\item \textbf{D-nuclear-Z>82}: Extension of the valley-of-stability theorem to $Z>82$ (OP15).
\item \textbf{Experimental confrontation}: (i) FLAG/lattice QCD for $\Delta m_{\mathrm{iso}}$ at $\pm 0.04$~MeV to distinguish $2.446$~MeV (D55) from $2.532$~MeV (D30). (ii) Submit D45 predictions to \textsc{BlackHawk}/\textsc{Isatis} community and contact Fermi-LAT for IGRB analysis in the 50--200\,MeV window.
\item \textbf{Journal submission}: D55 (Weinberg angle theorem), D53 (cosmological constant synthesis), D49 (London equation), D50 (Bekenstein--Hawking $\tfrac{1}{4}$), or D42 (H3 theorem) are strong candidates.
\item \textbf{Contact Recchia and Lenzi (University of Padova)}: Regarding the D41 nuclear confrontation and the D47 periodic table theorem.
\item \textbf{DL03}: Numerical enclosure of $n^*_{\mathrm{life}}$ from the gain function $R^1_{\mathrm{active}}$. The existence and ordering of all thresholds are established (DL02); this is the natural next step.
\end{enumerate}
...
\bibliography{DM_v24_references}

% ── Entrées à ajouter dans DM_v24_references.bib ─────────────────────────
%
% @misc{D56,
%   author       = {Laubscher, C.},
