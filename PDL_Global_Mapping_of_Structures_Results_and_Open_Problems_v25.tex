...
\normalsize Version 25 --- A Guide for Continuation of the Programme}
...
\subsection{What makes this version different from v24}
\label{subsec:diffv24}
Version~24 covered the corpus up to D56 and included the joint PDL--OFN bridge note N01. The present version~v25 incorporates two new documents of structural significance. Document D57 derives $\sin^2\theta_W^{(\mathrm{tree})} = \sin^2(\pi/R_e) = 1/4$ as an unconditional theorem of C1--C4 (two independent routes, proved equivalent via the Klein four-group $V_4$), establishes Hypothesis H$_{\mathrm{SU2}}$ (C1 forces $V_4$ trivial $\to$ effective group $S_3 \to \mathrm{Dic}_3 \subset \mathrm{SU}(2)$), and identifies $A_4/V_4 \cong \mathbb{Z}_3$ as the combinatorial origin of $\beta_1 = 3$. The joint note N01 (Laubscher \& Evdokimov) documents the PDL--OFN convergence on $\beta_1=3$ and poses Open Problem OP-OFN-1. The reader familiar with v24 should note: D57 and N01 added to corpus table; OP10 updated with tree-level theorem; OP-D57-1, OP-D57-2, OP-D57-3, OP-SU3, and OP-OFN-1 added; dependency diagram gains D57 node and H$_{\mathrm{SU2}}$ conjecture node; Recommended next documents updated; bibliography gains three entries (D57, N01, D-exp-E2-PDL).
...
\begin{openproblem}[OP-SU3 --- Derivation of $\mathrm{SU}(3)$ from the proton hierarchy]
\label{op:SU3}
Derive the $\mathrm{SU}(3)$ colour gauge group from the proton triplet $(u,u,d)$ and the three leakage cycles ($\beta_1=3$). Candidate mechanism: the three leakage cycles (exponents 23, 67, 997) correspond to the three $\mathrm{SU}(2)$ subalgebras of $\mathrm{su}(3)$; the quotient $A_4/V_4 \cong \mathbb{Z}_3$ corresponds to $\mathbb{Z}_3 \subset Z(\mathrm{SU}(3))$ (centre). The asymmetry $\Delta n = n_d - n_u = 4$ (D47, theorem) drives the breaking $\mathrm{SU}(3)\to\mathrm{SU}(2)\times\mathrm{U}(1)$. Document D58 (forthcoming) will address this. \textbf{Priority:} high. \textbf{Entry point:} D47, D51, D52, D57.
\end{openproblem}
...
\node[theorem] at ( 0.0,-12.0) (D57tree)  {$\sin^2\theta_W^{(\mathrm{tree})}$\\$=\sin^2(\pi/R_e)=1/4$\\(D57)};
\node[conjecture] at ( 3.5,-12.0) (HSU2)  {H$_{\mathrm{SU2}}$\\$S_3\to\mathrm{Dic}_3$\\$\subset\mathrm{SU}(2)$};
...
\begin{enumerate}
\item \textbf{DM v25}: The present document. Incorporates D57 ($\sin^2\theta_W^{(\mathrm{tree})}=1/4$ theorem, H$_{\mathrm{SU2}}$ conjecture, $A_4/V_4\cong\mathbb{Z}_3$), N01 (PDL--OFN bridge), and new open problems OP-OFN-1, OP-D57-1, OP-D57-2, OP-SU3.
\item \textbf{OP-D57-1 --- Lemma C1-$V_4$}: Proving formally that C1 forces $V_4$ trivial would promote H$_{\mathrm{SU2}}$ to a theorem. Immediately actionable from D46 and D57 Scripts~1--7. \textbf{Entry point:} D46, D57.
\item \textbf{D58 --- $\mathrm{SU}(3)$ from the proton hierarchy}: Derive the colour gauge group from the triplet $(u,u,d)$ and the three leakage cycles. Requires Colab verification of H$_{\mathrm{SU3}}$ before drafting. \textbf{Entry point:} D47, D51, D57.
...
\bibliography{DM_v25_references}
