...
\normalsize Version 27 --- A Guide for Continuation of the Programme}
...
The current corpus (D01--D59, plus auxiliary documents, the PDL-V series DL01--DL02, and the joint PDL--OFN bridge note N01) has reached the following state:
\begin{itemize}
...
\label{op:D59two}
The gauge group $\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)$ and its fundamental representation~$\mathbf{3}$ are established from C1--C4 (D46+D57+D58+D59). A structural argument suggests that the minimal covariant derivative $D_\mu = \partial_\mu - igA_\mu$ is the unique gauge-transport structure compatible with C4: any parallel transport on $K_4$ that is not equivariant under the derived gauge group introduces an asymmetry that C4 eliminates (survival principle --- C4 as the PDL analogue of Darwinian selection on transport structures). If formalised, this would make gauge dynamics a consequence of the axioms rather than a variational postulate. Currently a structural hypothesis motivating a future document. \textbf{Priority:} high. \textbf{Entry point:} D58, D59, D46, D49.
\end{openproblem}
...
% ── D59 node : fundamental representation 3
\node[theorem] at ( 8.5,-11.8) (D59node)  {$\mathrm{SU}(3)_c$ in $\mathbf{3}$\\$W=\{a{+}b{+}c=0\}$\\(D59)};
...
% D58 + D47 → D59
\draw[arr] (D58node.east)    -- (D59node.west);
\draw[arr] (D47node.north)   to[out=90, in=-90] (D59node.south);

% D59 → SMgauge (representation layer)
\draw[arr] (D59node.east)    to[out=0, in=-90] (SMgauge.south);
...
\begin{enumerate}
\item \textbf{DM v27}: The present document. Incorporates D59 (fundamental representation~$\mathbf{3}$ of $\mathrm{SU}(3)_c$, matter orientation from $\Delta n > 0$) and \texttt{PDL\_D59\_script1.py}.
\item \textbf{OP-D59-2 --- Minimal covariant derivative from C4}: Formalise the argument that $D_\mu = \partial_\mu - igA_\mu$ is the unique C4-compatible gauge transport on $K_4$. This would derive gauge dynamics from the survival principle without a variational postulate. \textbf{Entry point:} D58, D59, D49.
\item \textbf{OP-D57-1 --- Lemma C1-$V_4$}: Proving formally that C1 forces $V_4$ trivial would promote H$_{\mathrm{SU2}}$ to an unconditional theorem. \textbf{Entry point:} D46, D57.
\item \textbf{OP-OFN-1 --- Three leakage cycles and three fermion generations}: D59 strengthens the evidence (triplet $(T_2)$ labels the axes of $W$), but the formal derivation of three dynamical generation replicas requires a new mechanism. \textbf{Entry point:} N01, D57, D58, D59.
...
\bibliography{DM_v27_references}
