...
\normalsize Version 28 --- A Guide for Continuation of the Programme}
...
The current corpus (D01--D61, plus auxiliary documents, the PDL-V series DL01--DL02, and the joint PDL--OFN bridge note N01) has reached the following state:
\begin{itemize}
...
\end{itemize}
There is no longer any foundational open problem at the axiomatic level. The causal chain from C1--C4 to the cosmological constant $\Lambda_{\mathrm{PDL}}$ is complete. The equation of state of the coherence fluid (D54) is fully resolved. The Weinberg angle (D55) is derived as an unconditional theorem, closing OP10 at the level of $\theta_W$. The remaining open problems concern higher-level derivations (electroweak $W/Z$ mass ratio, quark mass spectrum, three fermion generations), extensions (superheavy nuclei, dark matter identification), and experimental confrontations. A new structural hypothesis OP-D59-2 is identified: the minimal covariant derivative $D_\mu = \partial_\mu - igA_\mu$ may be the unique C4-compatible gauge transport, making gauge dynamics a consequence of the survival principle.
\end{abstract}
...
\subsection{What makes this version different from v26}
\label{subsec:diffv26}
Version~26 covered the corpus up to D58 and established $\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)$ as an unconditional algebraic theorem of C1--C4. The present version~v27 adds Document~D59 and its supplementary verification script \texttt{PDL\_D59\_script1.py}, resolving two open problems of D58 and completing the first derivation of a gauge representation from purely combinatorial axioms.
 
\begin{itemize}
\item \textbf{New in v27 --- D59} constructs the fundamental representation $\mathbf{3}$ of $\mathrm{SU}(3)$ from C1--C4 and identifies the physical colour gauge group $\mathrm{SU}(3)_c$. The carrier space is the trace-zero plane $W = \{(a,b,c)\in\mathbb{C}^3 : a+b+c=0\}$, with axes canonically labelled by $V_4\setminus\{e\}$ (D58, Lemma~L2). Five lemmas verified by exhaustive exact-integer arithmetic in \texttt{PDL\_D59\_script1.py}~\cite{D59py}: (L1)~$S_3 = S_4/V_4$ acts faithfully and transitively on $V_4\setminus\{e\}$ (recalled from D58); (L2)~character decomposition $\chi_\pi = \chi_{\mathrm{trivial}} + \chi_{\mathrm{standard}}$, verified over all 24 elements of $S_4$; (L3)~$W = \{a+b+c=0\}$ is the unique $S_3$-invariant complement of $\mathbb{C}(1,1,1)$ in $\mathbb{C}^3$; (L4)~$A_2$ root system in $W$ with Cartan matrix $\bigl[\begin{smallmatrix}2&-1\\-1&2\end{smallmatrix}\bigr]$, confirmed by exact integer inner products; (L5)~the isospin asymmetry $\Delta n = n_d - n_u = 4 > 0$ (unconditional theorem of C4, D47) forces the canonical orientation selecting $\mathbf{3}$ over $\bar{\mathbf{3}}$, identifying the matter sector from the axioms. Exact identity: $k_3/(k_1+k_2) = R_e = 6$.
\item A documented negative result shows that the bijection between the leakage-cycle triplet $(T_2) = \{9,19,168\}$ and the orbit triplet $(T_3) = V_4\setminus\{e\}$ is physically motivated by the quintuplet but not $S_4$-equivariant.
\item Open problems resolved by D59: OP-D58-1 (physical identification of $\mathrm{SU}(3)_c$) and OP-D58-3 (fundamental representation $\mathbf{3}$ from C1--C4).
\item New structural hypothesis identified: the minimal covariant derivative $D_\mu = \partial_\mu - igA_\mu$ is the unique gauge-transport structure compatible with C4 (any non-equivariant transport introduces a cost eliminated by C4). Stated as OP-D59-2.
\end{itemize}
 
The reader familiar with v26 should note: D59-py and D59 added to corpus table; OP-D58-1 and OP-D58-3 marked resolved; OP-D59-1 (quark mass spectrum) and OP-D59-2 (minimal covariant derivative from C4) added; dependency diagram updated; Recommended next documents updated; bibliography gains two entries (\texttt{D59py}, \texttt{D59}).
...
\begin{openproblem}[OP-D61-1 --- Gauge boson masses from C1--C4]
\label{op:D61one}
...
% ── COL 0 : Axiomes
\node[axiom]   at ( 0.0,  0.0) (C1C4)   {C1--C4\\Axioms};
\node[axiom]   at ( 0.0, -2.0) (K4)     {$K_4$\\6 edges\\8 coh. configs};

% ── COL 1 : U(1) branch (D46)
\node[theorem] at ( 4.0,  0.0) (U1)     {$\Uone$ phase\\Hopf fibration\\(D46)};

% ── COL 1b : D57 Weinberg tree
\node[theorem] at ( 4.0, -2.0) (D57)    {$\sin^2\theta_W^{(\mathrm{tree})}=1/4$\\$192/768 = |V_4|/16$\\(D57)};

% ── COL 1c : D60 H_SU2
\node[theorem] at ( 4.0, -4.2) (D60)    {H$_{\mathrm{SU2}}$ theorem\\$G_{\mathrm{eff}}=S_4/V_4\cong S_3$\\(D60, C1+C2)};

% ── COL 2 : D58 SU(3)
\node[theorem] at ( 8.5, -2.0) (D58)    {$\SUthree$ algebraic\\Weyl group $A_2$\\(D58+C--K--W)};

% ── COL 2b : D55 Weinberg
\node[theorem] at ( 8.5,  0.0) (D55)    {$\theta_W=19\pi/119$\\(D55)};

% ── COL 3 : D59 representation 3
\node[theorem] at (12.5, -2.0) (D59)    {$\SUthree_c$ in $\mathbf{3}$\\$W=\{a{+}b{+}c=0\}$\\axes $=V_4\setminus\{e\}$\\(D59)};

% ── COL 3b : G_gauge as theorem
\node[theorem] at (12.5,  0.0) (Ggauge) {$\SUthree{\times}\SUtwo{\times}\Uone$\\THEOREM\\(D46+D60+D58+D59)};

% ── GAUGE TRANSPORT CHAIN (D61) — Row below
% alpha: C4 → Coh(K4)
\node[theorem] at ( 4.0, -6.5) (alpha)  {C4-admissible\\transports\\$\Coh\cong\mathbb{Z}_2^3$\\(D61-$\alpha$, script1)};

% beta: Z2^3 ⋊ V4
\node[theorem] at ( 8.5, -6.5) (beta)   {$\mathbb{Z}_2^3\rtimes V_4$\\order 32\\$F\cong V_4$=matchings\\(D61-$\beta$, script2)};

% gamma: consistency
\node[consist] at (12.5, -6.5) (gamma)  {Consistency\\$\to G_{\mathrm{gauge}}$\\(D61-$\gamma$, via D46--D60)};

% delta: D_mu via Utiyama
\node[ext]     at (12.5, -4.2) (Utiyama){Utiyama 1956\\(external lemma)};
\node[theorem] at (16.5, -5.5) (Dmu)    {$\Dmu=\partial_\mu-igA_\mu$\\UNIQUE\\(D61-$\delta$)};

% Negative results
\node[neg] at ( 0.5, -8.5) (neg1) {NEG: conj.\\$V_4$ order 4\\refuted};
\node[neg] at ( 4.0, -8.5) (neg2) {NEG: global\\flip $(-1,\ldots)$\\not admissible};
\node[neg] at ( 8.5, -8.5) (neg3) {NEG: $V_4$ faithful\\on $H_1(K_4;\mathbb{R})$\\(D58)};

% Open problems
\node[open] at (16.5, -2.0) (OP10c)  {OP10-c\\$W/Z$ ratio\\open};
\node[open] at (16.5,  0.0) (OPOFN)  {OP-OFN-1\\3 generations\\open};
\node[open] at (16.5, -8.5) (OP61c)  {OP-D61-3\\autonomous\\$G_{\mathrm{gauge}}$};

%% ══════════════════ FLÈCHES ══════════════════
\begin{pgfonlayer}{background}

% C1C4 → K4, U1, D57, D60
\draw[arr] (C1C4.south)   -- (K4.north);
...
% U1 → D55, Ggauge
\draw[arr] (U1.east)      -- (D55.west);
\draw[arr] (U1.south)     to[out=-90,in=90] (Ggauge.north);

% D57 → D58, Ggauge
\draw[arr] (D57.east)     -- (D58.west);
...
% D58 → D59, Ggauge
\draw[arr] (D58.east)     -- (D59.west);
\draw[arr] (D58.north)    to[out=90,in=180] (Ggauge.south);

% D59 → Ggauge
\draw[arr] (D59.north)    -- (Ggauge.south);

% D55 → Ggauge
\draw[arr] (D55.east)     to[out=0,in=90] (Ggauge.north);

% Ggauge → OP10c, OPOFN
\draw[arr] (Ggauge.east)  to[out=0,in=180] (OP10c.west);
...
\caption{Gauge sector dependency map of the PDL programme (v28). \textcolor{proofblue}{Blue solid}: unconditional theorems. \textbf{Blue dashed}: consistency result (arrow~$\gamma$ of D61: the transport structure is consistent with $G_\mathrm{gauge}$, but this arrow relies on D46--D60 rather than being a new derivation). Grey: external lemma (Utiyama 1956). \textcolor{openproblemred}{Red}: open problems. Orange: documented negative results (scientifically informative). The four arrows of the D61 causal chain are labelled $\alpha$--$\delta$. $\alpha$: C4 forces $\mathrm{Coh}(K_4)\cong\mathbb{Z}_2^3$ (2048 checks). $\beta$: $V_4$ acts faithfully on $\mathbb{Z}_2^3$ by automorphisms, $F\cong V_4$ fixed by the three perfect matchings (288 checks). $\gamma$: consistency with $G_\mathrm{gauge}$ (theorem via D46--D60). $\delta$: $D_\mu=\partial_\mu-igA_\mu$ unique, by Utiyama. The complete gauge sector --- group, representations, and dynamics --- is derived from C1--C4 with no variational postulate and no free parameter at the structural level.}
...
\begin{enumerate}
\item \textbf{DM v28}: The present document. Incorporates D60 (H$_{\mathrm{SU2}}$ unconditional theorem) and D61 ($D_\mu$ from C4, gauge sector complete).
\item \textbf{OP-D61-1 --- Gauge boson masses}: Derive $M_W/M_Z = \cos(19\pi/119)$ from C1--C4 (OP10-c). \textbf{Entry point:} D55, D57, D61.
\item \textbf{OP-D61-2 / OP-OFN-1 --- Three fermion generations}: Formalise the link between $\beta_1(K_4) = 3$, $A_4/V_4 \cong \mathbb{Z}_3$, orbit $\mathcal{O}_5$, and three dynamical generation replicas. \textbf{Entry point:} N01, D57, D59, D61.
...
\bibliography{DM_v28_references}
