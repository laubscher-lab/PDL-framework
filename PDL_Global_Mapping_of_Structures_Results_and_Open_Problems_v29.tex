...
% Version 29 — C. Laubscher % Supersedes DM v28 (10.5281/zenodo.20646905)
% ============================================================
...
\normalsize Version 29 --- A Guide for Continuation of the Programme}
...
The current corpus (D01--D63, plus auxiliary documents, the PDL-V series DL01--DL02, and the joint PDL--OFN bridge note N01) has reached the following state:
\begin{itemize}
...
The following list supersedes the open-problem list of v28. Problems are classified by priority and domain. OP1, OP4, OP8, OP11, OP12, OP13, OP14, OP-London, OP1-D35, OP-pressure, OP-D57-1, OP-D59-2, OP-SU3, OP-D58-1, OP-D58-3, and OP-D41-1-A have been fully resolved and are retained for historical reference only. OP-D59-1 is partially addressed by D63 (see below). OP-D61-1 is partially addressed by D62.
...
\node[theorem]    at (16.5, -1.0) (D62mass) {$M_Z/M_W$\\$=1/\cos(19\pi/119)$\\THEOREM (D62)};
\node[conjecture] at (16.5, -3.0) (D62conj) {$v$, $M_Z$, $M_H$\\conjectures\\(D62)};

% ── COL 4b : D63 quark masses [NEW v29]
\node[conjecture] at (20.5, -3.0) (D63node) {H\_mass: $m_d/m_u=2401/1104$\\H\_sea: $m_s,m_c,m_b,m_t$\\(D63, conjectures)};
\node[ext]        at (20.5, -1.0) (miso_ext){$\Delta m_{\mathrm{iso}}$\\(external, D31)};
...
\node[open] at (20.5,  0.5) (OPOFN)  {OP-OFN-1\\3 generations\\open};
...
\draw[arr] (D62mass.south) -- (D62conj.north);

% D63: D62conj + miso_ext → D63node
\draw[arr] (D62conj.east)  to[out=0,in=180] (D63node.west);
...
\caption{Complete list of falsifiable, parameter-free PDL predictions (v29). All predictions follow from the quintuplet and $\Delta m_{\mathrm{iso}}$ alone, with no additional free parameters. Predictions P1--P6 are derived from the analytical chain; P7--P8 follow from Conjecture~$H_B$ (D41); P9 is derived from D45; P10 from D55; P11--P17 from D62 and D63.}
...
\begin{enumerate}
\item \textbf{DM v29}: The present document. Incorporates D62 (gauge boson masses: $M_Z/M_W$ theorem, $v$, $M_H$ conjectures) and D63 (quark mass spectrum: H\_mass and H\_sea conjectures, structural identity $n_u - 1 = p_{k_1} = 23$).
...
\bibliography{DM_v29_references}
