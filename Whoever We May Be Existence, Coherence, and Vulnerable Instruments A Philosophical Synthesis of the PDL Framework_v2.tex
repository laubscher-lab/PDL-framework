...
This text does not present new theorems, numerical derivations, or empirical
predictions within the PDL framework. It is written as an interpretive and
philosophical companion to the technical corpus, reconstructing the ontological
stance that is implicit in the existing graph-theoretic constructions. Its
central aim is to make that stance explicit for two overlapping audiences:
researchers in the philosophy and foundations of physics, and scientifically
literate non-specialists willing to engage with conceptual arguments without
following every combinatorial proof.
...
Throughout what follows, three layers are interwoven. When the discussion concerns the formal PDL machinery itself, it will be described as belonging to the \emph{mathematical} layer. When it speaks about electrons, protons, fields, and constants, it should be read as operating at the \emph{physical} layer. When it reflects on notions such as existence, freedom, or vulnerability, it is explicitly situated at the \emph{philosophical} layer. Whenever needed, the text will signal which layer is in play and how a given passage connects back to the others.
...
\section*{PDL in plain language: a first intuition}

Before turning to the more systematic reconstruction of PDL, it is helpful to sketch in ordinary language what its basic ingredients amount to.

At the most stripped-down level, PDL starts from the idea that what exists is not a collection of featureless points in an empty space, but patterns of relations that can hold together on their own. A \emph{finite signed graph} is a simple way of encoding such patterns: a small network of abstract locations (vertices) joined by links (edges), where each link carries a sign, $+1$ or $-1$, standing for an elementary attitude of agreement or disagreement. A \emph{closure} is then a configuration of signs on such a graph that manages to be internally consistent, in the sense that most of its smallest circuits (triangles) agree with a prescribed rule of coherence.

To say that a closure is \emph{pulsating} is to say that it does not simply sit there as a static list of relations. Under a very simple update rule, its signs are repeatedly rearranged, yet after a finite number of steps -- typically two -- the whole pattern returns exactly to itself. In this minimal sense, the structure carries its own ``heartbeat'': a logical cycle by which distinction is lost and regained. The electron-like $(4,6)$ block introduced later in the paper is an example of such a pulsating closure.

One can think of these ingredients as a toy model of what it means for there to be something rather than nothing in a logically constrained universe. Instead of postulating a background spacetime and filling it with objects, PDL asks for the smallest possible relational structure that can sustain a recurring pattern of coherence without leaning on an external scaffolding. The remainder of the text unpacks how this very sparse starting point is meant to connect, step by step, to familiar physical entities and to the more philosophical claims about existence, vulnerability, and coherence.
...
(physical layer) The \((4,6)\) closure is, at this stage, a purely combinatorial object. It is the first finite signed graph that can sustain a non-trivial logical $2$-cycle while satisfying the PDL constraints of triangular coherence, minimal completeness, and optimisation. Nothing in its definition mentions charge, mass, spin, or spacetime. Yet, if PDL is to be more than a mathematical curiosity, such closures must eventually be confronted with the phenomenology of actual particles.
...
The move from the formal notion of coherence leakage to the philosophical language of transcendence requires an intermediate step. In the technical corpus, leakage is a strictly combinatorial quantity: the small but non-zero fraction of triangular closure conditions that cannot be satisfied simultaneously in an optimised configuration. In this paper, the term ``transcendence'' is used to name what this structural fact means once one reads the closures as candidates for real entities. The present section should therefore be read as an interpretive extrapolation of the leakage results, not as a new theorem: it spells out how a universe built from incomplete closures is, by construction, one in which no entity can be entirely self-contained.

(philosophical layer) On this reading, transcendence is not a separate realm beyond physics but a structural feature of any universe built from incomplete closures. No finite structure can fully contain the coherence it requires to persist; some part of that requirement must always be exported to, or borrowed from, a wider network of relations. The notion of leakage lends itself naturally to a philosophical vocabulary that has often been reserved for theology or metaphysics. In collaboration and dialogue with the Theory of Objectivity, PDL has explored the idea that the impossibility of perfect closure can be read as a disciplined notion of transcendence.\cite{Laubscher_PDL_TO_Dialogue_2024,Laubscher_Existence_PulsatingClosure_2026} Here, transcendence does not mean a separate realm beyond the physical world; it denotes the structural fact that any finite closure necessarily refers beyond itself, because it cannot fully absorb the coherence requirements it generates.
...
The proposal sketched so far remains tightly tethered to the structural features of PDL closures and their coherence budgets. The next two subsections move more decisively into interpretive and existential territory. They should be taken as philosophical extrapolations of the PDL ontology of pulsating, leaky closures, not as derivations of human experience from finite signed graphs.

\subsection{Humans as vulnerable instruments}

(philosophical layer) To apply the language of closures and coherence budgets to human beings is to take a deliberate step beyond the domain in which PDL currently offers detailed constructions. The justification for this step is not that there exists a one-to-one mapping between neurons and signed graphs, but that any physically real organism must, on the PDL picture, be built from the same kind of finite, pulsating, leaky closures as electrons and protons. The present discussion asks what it means to inhabit such a layered closure as a conscious agent. It is offered as a way of reading familiar features of human vulnerability in the light of a structural dependence on wider coherence, not as a reduction of persons to graph-theoretic objects.
...
\subsection*{The core idea in one paragraph}

(philosophical layer) Seen as a whole, the proposal of this paper can be stated succinctly. The PDL framework treats existence as the realisation of finite, pulsating closures on signed graphs: minimal patterns of relations that can repeatedly restore their own coherence without relying on an unarticulated background. Coherence, in this setting, is the basic currency of reality, and physical quantities such as mass, energy, and certain constants can be read as coarse summaries of how coherence budgets are allocated across internal structure, active surfaces, and leakage into surrounding networks. Selection then refers to the fact that only a very small set of architectures -- from the $(4,6)$ electron prototype to the proton and beyond -- satisfy the constraints of minimal closure in a logically constrained universe. To say that we, whoever we may be, are vulnerable instruments of a relentless logic of coherence is to say that our lives unfold inside such selected, incomplete closures, dependent at every moment on patterns of consistency that run far beyond what we can see or control.
